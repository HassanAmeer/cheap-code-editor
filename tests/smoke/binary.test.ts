...
describe("binary smoke tests", () => {
	...
	it("--version exits cleanly", () => {
		const result = runBinary({
			args: ["--version"],
			extraEnv: { CHEAP_API_KEY: "smoke-test-dummy" },
		})
	...
	it("--help shows cheap subcommands, harness flags, and env vars (no pi internals)", () => {
		const result = runBinary({
			args: ["--help"],
			extraEnv: { CHEAP_API_KEY: "smoke-test-dummy" },
		})
		...
		expect(result.stdout).toContain("cheap setup")
		expect(result.stdout).toContain("cheap claude")
		expect(result.stdout).toContain("cheap opencode")
		expect(result.stdout).toContain("cheap cursor")
		expect(result.stdout).toContain("cheap openclaw")
		expect(result.stdout).toContain("cheap gsd2")
		// Curated harness flags forwarded to pi
		expect(result.stdout).toContain("--provider")
		expect(result.stdout).toContain("--mode")
		expect(result.stdout).toContain("--continue")
		// Cheap-only env vars
		expect(result.stdout).toContain("CHEAP_API_KEY")
		// Pi-internal extension management commands and provider-specific env
		// vars must not leak into cheap's help screen.
	...
	it("version subcommand prints version + platform without launching the harness", () => {
		...
		expect(result.stdout).toMatch(/^cheap (?:dev|\d+\.\d+\.\d+)/)
	...
	describe("--export", () => {
		...
		beforeEach(() => {
			workDir = mkdtempSync(join(tmpdir(), "cheap-smoke-export-"))
		})
		...
		it("exports a session to HTML using staged template assets", () => {
			...
			expect(html).toContain("window.__CHEAP_VERSION")
			expect(html).toContain('class="info-label">Version:')
		})
	})

	it.skipIf(!process.env.CHEAP_API_KEY)("sends a request to a model via -p flag", { retry: 2 }, () => {
		const result = runBinary({
			args: ["--debug-prompts", "-p", "respond with only the word hello"],
			extraEnv: { CHEAP_API_KEY: process.env.CHEAP_API_KEY as string },
		})
