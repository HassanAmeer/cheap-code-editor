// End-to-end checks that a real cheap run spawning a real Agent leaves parent
...
describe("Agent session tracking smoke tests", () => {
	...
	beforeEach(() => {
		sessionDir = mkdtempSync(join(tmpdir(), "cheap-agent-session-"))
	})
	...
	it.skipIf(!process.env.CHEAP_API_KEY)(
