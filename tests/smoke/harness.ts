 ...
 * cheap binary with a sandboxed HOME. The binary computes its agent
 * config dir as HOME/.config/cheap/harness, so we expose that derived
...
export const BINARY_PATH = resolve("dist/bin/cheap")
export const PACKAGE_DIR = resolve("dist/share/cheap")

let tempHome: string | undefined

beforeAll(() => {
	tempHome = mkdtempSync(join(tmpdir(), "cheap-smoke-home-"))
	const configDir = join(tempHome, ".config", "cheap")
	...
	// the dummy CHEAP_API_KEY gets a 401 from the live metadata endpoint. Without
	// this, interactive smoke tests would fail to boot the binary in CI.
	const agentDir = join(tempHome, ".config", "cheap", "harness")
 ...
 * Matches the logic in src/cli.ts: resolve(homedir(), ".config", "cheap", "harness").
 */
export function getAgentDir(): string {
	return join(getTempHome(), ".config", "cheap", "harness")
}
...
export interface InteractiveSession {
	...
	pty: IPty
	/** Everything cheap has printed so far, ANSI escapes intact. */
	output(): string
	...
	/** Type `data` into cheap as if from a keyboard. */
...
/** Spawn cheap under a real PTY so its interactive TUI boots. Prefer `runBinary` for non-interactive checks — use this only when you need to drive the editor/prompt. */
export function spawnInteractive(opts: RunInteractiveOptions = {}): InteractiveSession {
	...
	const pty = ptySpawn(BINARY_PATH, [], {
		...
		env: {
			...
			PI_PACKAGE_DIR: PACKAGE_DIR,
			TERM: "xterm-256color",
			CHEAP_API_KEY: "smoke-test-dummy",
			...extraEnv,
		},
	})
