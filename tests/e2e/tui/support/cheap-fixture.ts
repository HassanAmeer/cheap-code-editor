...
const REPO_ROOT = process.env.CHEAP_REPO_ROOT
	? resolve(process.env.CHEAP_REPO_ROOT)
...
const DEBUG_ARTIFACTS = process.env.CHEAP_TUI_E2E_DEBUG === "1"

/** Provider key written into models.json and passed to the cheap CLI; the two must agree. */
export const FAKE_PROVIDER = "fake"

export const BINARY_PATH = resolve(REPO_ROOT, "dist/bin/cheap")
export const PACKAGE_DIR = resolve(REPO_ROOT, "dist/share/cheap")
const INITIAL_SURVEY_ID = "019e87cc-5033-0000-d9bd-5e6501640b6e"

export interface CheapFixture {
	homeDir: string
	workDir: string
	agentDir: string
...
interface CreateCheapFixtureOptions {
	models?: FakeModel[]
...
export async function createCheapFixture(options: CreateCheapFixtureOptions): Promise<CheapFixture> {
	...
	const homeDir = mkdtempSync(join(tmpdir(), "cheap-tui-home-"))
	const workDir = mkdtempSync(join(tmpdir(), "cheap-tui-work-"))
	// Tear down server + temp dirs if any setup step throws.
	try {
		if (options.gitInit) execFileSync("git", ["init", "-q"], { cwd: workDir })
		const configDir = join(homeDir, ".config", "cheap")
...
export function launchCheap(terminal: Terminal, fixture: CheapFixture, extraArgs: string[] = []): void {
	// CHEAP_PERMISSIONS=yolo skips every permission check (rules, denylist,
...
export async function stopCheap(terminal: Terminal): Promise<void> {
...
/** Create fixture, launch cheap, wait for ready, run `body`, always tear down (artifact on throw). */
export async function runCheapSession(
	terminal: Terminal,
	options: CreateCheapFixtureOptions & { artifactName: string },
	body: (fixture: CheapFixture, trace: TuiScenarioTrace) => Promise<void>,
): Promise<void> {
	const { artifactName, ...fixtureOptions } = options
	const fixture = await createCheapFixture(fixtureOptions)
	...
	try {
		launchCheap(terminal, fixture, fixtureOptions.extraArgs ?? [])
		...
		try {
			await stopCheap(terminal)
		} catch (stopError) {
			process.stderr.write(`[tui-e2e] stopCheap failed: ${String(stopError)}\n`)
		}
		try {
			await fixture.stop()
		} catch (stopError) {
		...
		JSON.stringify(
			{
				providers: {
					[FAKE_PROVIDER]: {
						...
						authHeader: true,
						headers: { "User-Agent": "cheap/tui-e2e" },
