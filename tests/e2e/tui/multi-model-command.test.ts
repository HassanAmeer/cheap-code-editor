...
import { PROMPT_READY, TUI_TEST_CONFIG, runCheapSession } from "./support/cheap-fixture.js"
...
test("/multi-model metadata editor saves tier/vision/description", async ({ terminal }) => {
	await runCheapSession(
		...
		async (fixture, trace) => {
			...
			// default orchestrator is "cheap-dev/minimax-m3" and it has
			// builtin metadata, so the wizard will offer "keep current (X)"
			// options alongside the explicit choices.
			await waitForText(terminal, "cheap-dev/minimax-m3", { timeoutMs: INPUT_TIMEOUT_MS })
			...
			expect(viewText(terminal)).toContain("cheap-dev/minimax-m3")
			terminal.submit("")
			// The submenu title is `${ref} — metadata`. Matching the full
			// `cheap-dev/minimax-m3 — metadata` string avoids matching the
			// picker's own "Choose a model to edit metadata" header.
			await waitForText(terminal, "cheap-dev/minimax-m3 — metadata", { timeoutMs: INPUT_TIMEOUT_MS })
			...
			await waitForText(terminal, /Metadata saved for cheap-dev\/minimax-m3\./, { timeoutMs: INPUT_TIMEOUT_MS })
			...
			const settingsPath = `${fixture.homeDir}/.config/cheap/harness/settings.json`
			const settings = await readSettingsJson(settingsPath)
			const meta = settings.modelMetadata?.["cheap-dev/minimax-m3"]
...
test("/multi-model builder role opens toggle-select with current assignment pre-checked", async ({ terminal }) => {
	await runCheapSession(
		...
		async (_fixture, trace) => {
			...
			expect(view).toMatch(/\[x\]\s+cheap-dev\//)
