 ...
 * cheap with `--provider ollama --model <discovered-id>` so the orchestrator
 ...
 * fixture (cheap-fixture.ts) starts the mock and injects OLLAMA_HOST into the
 ...
 *       2. `dist/bin/cheap --list-models` at runtime (proves pi-mono's
 ...
 *          in earlier attempts; see `.cheap/ferments/.../docs/step-4-design.md`.
...
test("chat round-trips through ollama when launched with --provider ollama", async ({ terminal }) => {
	...
	try {
		// Launch cheap with --provider ollama --model <first-ollama-model>
		launchCheap(terminal, fixture, ["--provider", "ollama", "--model", "ollama/llava:13b"])
