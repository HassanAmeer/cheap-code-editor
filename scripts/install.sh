#!/usr/bin/env bash
#
# Install the cheap coding-harness CLI from the latest GitHub release.
#
# Usage:
#   curl -fsSL https://github.com/castai/cheap/releases/latest/download/install.sh | bash
#
# Optional env:
#   CHEAP_INSTALL_DIR  Override install dir. Defaults to /usr/local/bin if
#                      writable, else $HOME/.local/bin (with a PATH hint).
#   CHEAP_VERSION      Pin a specific version tag (e.g. v0.2.0). Defaults
...
REPO="${CHEAP_REPO_OVERRIDE:-castai/cheap}"
VERSION="${CHEAP_VERSION:-latest}"

echo -e "${BLUE}Installing Cheap from ${REPO}${VERSION:+ (${VERSION})}…${NC}"
...
if [ "$VERSION" = "latest" ]; then
	BINARY_URL="https://github.com/${REPO}/releases/latest/download/cheap_${OS}_${ARCH}.tar.gz"
else
	BINARY_URL="https://github.com/${REPO}/releases/download/${VERSION}/cheap_${OS}_${ARCH}.tar.gz"
fi

TEMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TEMP_DIR"' EXIT

echo -e "${BLUE}Downloading cheap for ${OS}/${ARCH}…${NC}"
if ! curl -fsSL "$BINARY_URL" | tar -xzf - -C "$TEMP_DIR"; then
	echo -e "${RED}Failed to download cheap from $BINARY_URL${NC}" >&2
	echo "Please check that the release exists at:" >&2
	echo "  https://github.com/${REPO}/releases" >&2
	exit 1
fi

# The release tarball contains bin/cheap and share/cheap/.
if [ ! -f "$TEMP_DIR/bin/cheap" ]; then
	echo -e "${RED}Archive did not contain a 'bin/cheap' binary.${NC}" >&2
	exit 1
fi
chmod +x "$TEMP_DIR/bin/cheap"
...
if [ -n "${CHEAP_INSTALL_DIR:-}" ]; then
	INSTALL_DIR="$CHEAP_INSTALL_DIR"
...
mkdir -p "$INSTALL_DIR"
INSTALL_PATH="$INSTALL_DIR/cheap"
mv "$TEMP_DIR/bin/cheap" "$INSTALL_PATH"

# Install share files (themes, export templates, etc.) if present.
if [ -d "$TEMP_DIR/share/cheap" ]; then
	mkdir -p "$DATA_DIR"
	rm -rf "$DATA_DIR/cheap"
	mv "$TEMP_DIR/share/cheap" "$DATA_DIR/cheap"
fi

echo ""
echo -e "${GREEN}✓ Installed cheap to ${INSTALL_PATH}${NC}"
...
if [ "$NEEDS_PATH_SETUP" = "yes" ] || { [ "$NEEDS_PATH_SETUP" = "maybe" ] && ! command -v cheap >/dev/null 2>&1; }; then
	case "${SHELL:-}" in
	*/fish*)
		echo ""
...
echo ""
echo -e "${BLUE}Next:${NC} run ${GREEN}cheap${NC} and follow the first-time wizard to configure your API key and migrate your skills and MCPs"
