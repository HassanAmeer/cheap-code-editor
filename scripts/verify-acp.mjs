#!/usr/bin/env node
// Scripted ACP client that drives cheap --mode acp through handshake, newSession, prompt.
...
async function main() {
	...
	try {
		...
		// Tool-call test: prompt cheap to invoke a tool and verify tool_call + tool_call_update flowed through.
		...
		// global timeout. /tags is a cheap extension command (src/extensions/tags.ts) so it
