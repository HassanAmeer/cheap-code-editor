...
//
// Both scenarios drive a real LLM via stdio JSON-RPC; CHEAP_API_KEY must be
// in env. Companion to verify-acp.mjs / verify-acp-stop.mjs.
//
// Usage: node scripts/verify-acp-load.mjs [path-to-cheap-binary]
...
const binary = process.argv[2] ?? "./dist/bin/cheap"
...
async function scenarioCrossToolRoundTrip() {
	...
	// CLI leg: cheap reopens the same session via --session <id> and adds turn 2.
	...
	process.stderr.write(`[B2] launching cheap --session ${sessionId}\n`)
