...
//                                   (getThemesDir, getExportTemplateDir) relative to cheap's project root
//
// default (used by `build-binary`): theme files from node_modules → dist/share/cheap/theme/
//                                   export-html templates → dist/share/cheap/export-html/
//                                   plus package.json → dist/share/cheap/
...
const themeDest = isDev
	...
	: join(projectRoot, "dist", "share", "cheap", "theme")
const exportHtmlDest = isDev
	? join(projectRoot, "src", "core", "export-html")
	: join(projectRoot, "dist", "share", "cheap", "export-html")
...
// Post-process export HTML template to inject the cheap version number.
...
templateHtml = templateHtml.replace(
	...
	`<script>
(function() {
  window.__CHEAP_VERSION = ${JSON.stringify(appVersion)};
  const container = document.querySelector('.header-info');
  if (!container) return;
  const el = document.createElement('div');
  el.className = 'info-item';
  el.innerHTML = '<span class="info-label">Version:</span><span class="info-value">' + window.__CHEAP_VERSION + '</span>';
...
// cheap's own themes live outside node_modules — copy them alongside the upstream themes
const cheapThemesSrc = join(projectRoot, "themes")
const cheapThemeFiles = readdirSync(cheapThemesSrc).filter((f) => f.endsWith(".json"))
for (const file of cheapThemeFiles) {
	cpSync(join(cheapThemesSrc, file), join(themeDest, file))
}

if (!isDev) {
	cpSync(join(projectRoot, "package.json"), join(projectRoot, "dist", "share", "cheap", "package.json"))

	// Copy custom OAuth page templates
	const oauthSrc = join(projectRoot, "resources", "oauth")
	const oauthDest = join(projectRoot, "dist", "share", "cheap", "oauth")
	...
	const proxyHelperBinDest = join(projectRoot, "dist", "share", "cheap", "bin")
	...
	// 	join(projectRoot, "dist", "share", "cheap", "teleport-proxy.js"),
	// )
}
