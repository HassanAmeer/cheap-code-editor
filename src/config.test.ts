...
describe("loadConfig", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "cheap-test-"))
	...
	it("reads project config from .cheap/config.json overriding global", () => {
		const globalDir = mkdtempSync(join(tmpdir(), "cheap-test-"))
		const projectDir = mkdtempSync(join(tmpdir(), "cheap-test-"))
		const globalPath = join(globalDir, "config.json")
		const projectPath = join(projectDir, ".cheap", "config.json")
	...
	it("falls back to global when .cheap/config.json does not exist", () => {
		const globalDir = mkdtempSync(join(tmpdir(), "cheap-test-"))
		const projectDir = mkdtempSync(join(tmpdir(), "cheap-test-"))
		const globalPath = join(globalDir, "config.json")
	...
	it("does not walk up the directory tree from a subfolder", () => {
		const globalDir = mkdtempSync(join(tmpdir(), "cheap-test-"))
		const rootDir = mkdtempSync(join(tmpdir(), "cheap-test-"))
		const subDir = join(rootDir, "src", "components")
		const globalPath = join(globalDir, "config.json")
		const rootProjectPath = join(rootDir, ".cheap", "config.json")
		...
		// cwd is a subfolder that does NOT have .cheap/config.json
		const config = loadConfig({ configPath: globalPath, cwd: subDir })
		// Should NOT pick up rootDir/.cheap/config.json
...
describe("writeApiKey", () => {
	...
	it("can persist the Cheap API endpoint with the API key", () => {
		writeApiKey("sekrit-42", configPath, { llmEndpoint: " https://custom.cheap.example " })
		const raw = JSON.parse(readFileSync(configPath, "utf-8"))
		expect(raw.apiKey).toBe("sekrit-42")
		expect(raw.llmEndpoint).toBe("https://custom.cheap.example")
	})
...
describe("writeDeviceId", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "cheap-test-device-"))
		configPath = join(tempDir, "config.json")
	})
...
describe("readTelemetryConfig", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "cheap-telemetry-test-"))
		configPath = join(tempDir, "config.json")
		savedApiKey = process.env.CHEAP_API_KEY
		savedTelemetryEnabled = process.env.CHEAP_TELEMETRY_ENABLED
		// biome-ignore lint/performance/noDelete: env var must be deleted, not set to "undefined"
		delete process.env.CHEAP_API_KEY
		// biome-ignore lint/performance/noDelete: env var must be deleted, not set to "undefined"
		delete process.env.CHEAP_TELEMETRY_ENABLED
	})

	afterEach(() => {
		rmSync(tempDir, { recursive: true, force: true })
		if (savedApiKey !== undefined) process.env.CHEAP_API_KEY = savedApiKey
		// biome-ignore lint/performance/noDelete: env var must be deleted, not set to "undefined"
		else delete process.env.CHEAP_API_KEY
		if (savedTelemetryEnabled !== undefined) process.env.CHEAP_TELEMETRY_ENABLED = savedTelemetryEnabled
		// biome-ignore lint/performance/noDelete: env var must be deleted, not set to "undefined"
		else delete process.env.CHEAP_TELEMETRY_ENABLED
	})
	...
	it("injects User-Agent into telemetry headers", () => {
		...
		expect(config.headers["User-Agent"]).toMatch(/^cheap\//)
	})
...
describe("getActiveVendorSkillPaths", () => {
	...
	beforeEach(() => {
		...
		mockHome = mkdtempSync(join(tmpdir(), "cheap-vendor-test-"))
	...
	it("returns vendor paths when vendor dir exists and no sentinel", () => {
		mkdirSync(join(mockHome, ".config", "cheap", "vendor", "superpowers", "skills"), { recursive: true })
	...
	it("returns empty array when sentinel exists in harness skills dir", () => {
		mkdirSync(join(mockHome, ".config", "cheap", "harness", "skills", "using-superpowers"), { recursive: true })
		writeFileSync(join(mockHome, ".config", "cheap", "harness", "skills", "using-superpowers", "SKILL.md"), "")
...
describe("buildSkillPathOptions", () => {
	...
	beforeEach(() => {
		...
		tempDir = mkdtempSync(join(tmpdir(), "cheap-skillpaths-"))
	})
