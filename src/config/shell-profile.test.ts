...
describe("exportEnvToShellProfile", () => {
	...
	beforeEach(() => {
		...
		tmp = realpathSync(mkdtempSync(join(tmpdir(), "cheap-shell-test-")))
	...
	it("appends export to .zshrc, preserving existing lines", () => {
		...
		const path = exportEnvToShellProfile("CHEAP_API_KEY", "test-key-123")
		expect(path).toBe(join(tmp, ".zshrc"))

		const content = readFileSync(path as string, "utf-8")
		expect(content).toContain("export CHEAP_API_KEY=test-key-123")
		expect(content).toContain("alias ll='ls -la'")
	})

	it("updates an existing export line in place", () => {
		process.env.SHELL = "/bin/zsh"
		writeFileSync(join(tmp, ".zshrc"), "# config\nexport CHEAP_API_KEY=old-key\nalias ll='ls -la'\n", "utf-8")

		exportEnvToShellProfile("CHEAP_API_KEY", "new-key-456")
		const content = readFileSync(join(tmp, ".zshrc"), "utf-8")
		expect(content).toContain("export CHEAP_API_KEY=new-key-456")
	...
	it("does not duplicate the export line on repeated calls", () => {
		...
		exportEnvToShellProfile("CHEAP_API_KEY", "key1")
		exportEnvToShellProfile("CHEAP_API_KEY", "key1")

		const content = readFileSync(join(tmp, ".zshrc"), "utf-8")
		const matches = content.split("\n").filter((l) => l === "export CHEAP_API_KEY=key1").length
	...
	it("uses fish's set -gx syntax for fish shells", () => {
		...
		const path = exportEnvToShellProfile("CHEAP_API_KEY", "fish-key")
		expect(path).toBe(join(fishDir, "config.fish"))

		const content = readFileSync(path as string, "utf-8")
		expect(content).toContain("set -gx CHEAP_API_KEY fish-key")
	})

	it("creates the profile when it does not yet exist", () => {
		process.env.SHELL = "/bin/zsh"
		const path = exportEnvToShellProfile("CHEAP_API_KEY", "new-key")
		expect(path).toBe(join(tmp, ".zshrc"))

		const content = readFileSync(path as string, "utf-8")
		expect(content).toContain("export CHEAP_API_KEY=new-key")
	})

	it("returns null when no shell can be detected and no profile exists", () => {
		process.env.SHELL = ""
		const path = exportEnvToShellProfile("CHEAP_API_KEY", "key")
	...
	it("follows symlinks to write the underlying file, leaving the link intact", () => {
		...
		const path = exportEnvToShellProfile("CHEAP_API_KEY", "sym-key")
		expect(path).toBe(realFile)

		expect(readFileSync(realFile, "utf-8")).toContain("export CHEAP_API_KEY=sym-key")
	...
	it("rejects non-UTF-8 content rather than corrupting the file", () => {
		...
		expect(() => exportEnvToShellProfile("CHEAP_API_KEY", "key")).toThrow(/non-UTF-8/)
	})
})
