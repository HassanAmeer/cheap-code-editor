...
describe("resource store", () => {
	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "cheap-resources-"))
	...
	it("uses CHEAP_CODING_AGENT_DIR settings.json when set", () => {
		process.env.CHEAP_CODING_AGENT_DIR = dir
	...
	it("persists resource overrides without clobbering unrelated settings", () => {
		const path = tempSettingsPath()
		writeJson(path, { theme: "cheap-minimal", resources: { "tools.web_fetch": false } })
		...
		expect(JSON.parse(readFileSync(path, "utf-8")).theme).toBe("cheap-minimal")
