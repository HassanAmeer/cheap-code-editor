...
async function installRtkUnlocked(): Promise<RtkInstallResult> {
	...
	const tempDir = mkdtempSync(join(tmpdir(), "cheap-rtk-"))
	try {
...
function rtkInstallDir(): string {
	return join(process.env.CHEAP_CODING_AGENT_DIR ?? join(homedir(), ".config", "cheap", "harness"), "rtk")
}

function globalBinDir(): string {
	if (process.platform === "win32") {
		return join(process.env.LOCALAPPDATA ?? join(homedir(), "AppData", "Local"), "Programs", "cheap", "bin")
	}
