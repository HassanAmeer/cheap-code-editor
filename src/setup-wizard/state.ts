...
/**
 * How tool subcommands talk to cheap at runtime.
 *
 * - `override` (recommended): write cheap settings into each tool's own
 *   config file (`~/.config/opencode/opencode.json`, `~/.claude/settings.json`,
 *   …). The tool keeps working standalone afterwards.
 * - `inject`: launch via `cheap <tool>` only — env vars are set per-process,
