...
const CHEAP_API_KEY_ENV = "CHEAP_API_KEY"
 ...
 * `cheap <tool>` will be wired to launch.
 *
 * Once writes finish, we also append `export CHEAP_API_KEY=…` to the
 * user's shell profile so the key is available to future shells without
 * having to run `cheap setup` again.
 */
export async function runDoneStep(state: WizardState): Promise<ApplyOutcome> {
	...
	const agentDir = process.env.CHEAP_CODING_AGENT_DIR ?? resolve(process.env.HOME ?? "~", ".config/cheap-coding-agent")
	...
	// Best-effort: persist CHEAP_API_KEY to the user's shell profile so
	// it's available in future shells. We never fail the wizard on this —
	// the API key already lives in ~/.config/cheap/config.json, the env
	// var is just a convenience.
	const shellExport = state.apiKey ? exportEnvToShellProfile(CHEAP_API_KEY_ENV, state.apiKey) : { path: null }
	if (shellExport.path) {
		log.info(`${CHEAP_API_KEY_ENV} exported to ${shellExport.path}`)
	} else if (shellExport.error) {
		log.warn(`Could not export ${CHEAP_API_KEY_ENV} to shell profile: ${shellExport.error}`)
	...
	const summaryLines = [
		...
		outcome.failures.length > 0
			...
			: "",
		shellExport.path ? `${CHEAP_API_KEY_ENV}: exported to ${shellExport.path}` : "",
	].filter((l) => l.length > 0)
