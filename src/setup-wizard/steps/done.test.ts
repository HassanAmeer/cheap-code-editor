...
describe("runDoneStep", () => {
	...
	beforeEach(() => {
		...
		installRtkMock.mockResolvedValue({
			version: "v1.2.3",
			binaryPath: "/tmp/cheap-test/rtk",
			linkPath: "/tmp/cheap-test/bin/rtk",
		})
	...
	it("exports CHEAP_API_KEY to the user's shell profile", async () => {
		const tool = getClaudeCodeTool()
		const writeSpy = vi.spyOn(tool, "write").mockResolvedValue()
		...
		expect(zshrc).toContain("export CHEAP_API_KEY=test-key")
