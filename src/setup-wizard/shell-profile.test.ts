...
describe("exportEnvToShellProfile", () => {
	...
	it("appends export to zshrc", () => {
		...
		const result = exportEnvToShellProfile("CHEAP_API_KEY", "test-key-123")
	...
	it("updates existing export with new value", () => {
		...
		const result = exportEnvToShellProfile("CHEAP_API_KEY", "new-key-456")
		const content = readFileSync(result.path as string, "utf-8")
	...
	it("does not duplicate on repeated calls", () => {
		...
		const matches = content.split("\n").filter((line) => line === "export CHEAP_API_KEY=key1")
	...
	it("fish shell uses set -gx syntax", () => {
		...
		const result = exportEnvToShellProfile("CHEAP_API_KEY", "fish-key")
	...
	it("creates profile if it does not exist", () => {
		...
		const result = exportEnvToShellProfile("CHEAP_API_KEY", "new-key")
		expect(result.path).toBe(join(tmpHome, ".zshrc"))
	...
	it("returns null path when shell is unknown and no profiles exist", () => {
		...
		const result = exportEnvToShellProfile("CHEAP_API_KEY", "key")
	...
	it("follows symlinks", () => {
		...
		const result = exportEnvToShellProfile("CHEAP_API_KEY", "sym-key")
		expect(result.path).toBe(realFile)

		const content = readFileSync(realFile, "utf-8")
		expect(content).toContain("export CHEAP_API_KEY=sym-key")
