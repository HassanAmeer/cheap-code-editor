...
describe("authenticateViaBrowser", () => {
	...
	it("returns the token when the callback succeeds", async () => {
		...
		const promise = authenticateViaBrowser({ webAppUrl: "https://app.cheap.dev" })
		await vi.waitFor(() => {
		...
		expect(browserUrl).toMatch(/https:\/\/app\.cheap\.dev\/cli-auth\?callback=/)
	...
	it("aborts the callback wait when the signal fires (e.g. login dialog cancelled)", async () => {
		...
		const promise = authenticateViaBrowser({ webAppUrl: "https://app.cheap.dev", signal: controller.signal })
	...
	it("does not open the browser when the signal is already aborted", async () => {
		...
		await expect(
			authenticateViaBrowser({ webAppUrl: "https://app.cheap.dev", signal: controller.signal }),
