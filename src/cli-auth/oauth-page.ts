// Branded OAuth callback pages for the Cheap-account browser login.
//
...
// `CHEAP_OAUTH_TEMPLATE_DIR` (resources/oauth/{success,error}.html, set in
...
// sharing the template files the Cheap-account login and pi's subscription
...
// minimal *unbranded* page, deliberately not a Cheap- or Pi-branded fallback,
...
function renderPage(options: PageOptions): string {
	const templateDir = process.env.CHEAP_OAUTH_TEMPLATE_DIR
	if (templateDir) {
		try {
