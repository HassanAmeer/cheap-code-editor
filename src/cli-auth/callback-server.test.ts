...
describe("startCallbackServer", () => {
	...
	it("serves the branded success template when CHEAP_OAUTH_TEMPLATE_DIR is set", async () => {
		const previous = process.env.CHEAP_OAUTH_TEMPLATE_DIR
		process.env.CHEAP_OAUTH_TEMPLATE_DIR = resolve(repoRoot, "resources", "oauth")
		try {
			const state = generateState()
			const server = await startCallbackServer(state)
			...
			// in the minimal unbranded fallback, so this proves the Cheap-account
			...
			if (previous === undefined) {
				...
				process.env.CHEAP_OAUTH_TEMPLATE_DIR = previous
			}
		}
	})
