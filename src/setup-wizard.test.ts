...
describe("discoverCommandFiles", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "cheap-cmd-test-"))
	})
...
describe("discoverSubdirectoryCommandFiles", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "cheap-subcmd-test-"))
	})
...
describe("migrateCommandToPrompt", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "cheap-migrate-test-"))
...
describe("runSetupWizard", () => {
	...
	it("returns a cancelled result when optional custom skill input is cancelled", async () => {
		clackMock.multiselect.mockResolvedValueOnce([".config/cheap/harness/skills"])
	...
	it("marks successful setup results as not cancelled", async () => {
		...
		expect(result).toEqual({
			cancelled: false,
			skillPaths: [".config/cheap/harness/skills"],
