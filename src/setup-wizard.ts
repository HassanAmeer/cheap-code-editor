...
async function runMigrationPhase(d: MergedDiscovery): Promise<MigrationPhaseResult> {
	...
	const action = await clack.select<MigrationAction>({
		message: "Migrate MCP servers to Cheap?",
		options: [
...
export async function runSetupWizard(options: {
	...
	clack.intro("Cheap first-time setup")
	...
	if (merged?.hasAnythingMigratable) {
		...
		if (action === "migrate") {
			...
			if (serverCount > 0) {
				...
				clack.log.success(`Migrated ${serverCount} MCP server(s) to Cheap.`)
			}
			...
			if (commandsCopied > 0) {
				clack.log.success(`Migrated ${commandsCopied} command(s) to Cheap prompts.`)
			}
			migrationRan = true
	...
	if (options.needsSkillsSetup) {
		clack.note(
			"Cheap will look for skill files in the selected directories.\n" +
	...
	try {
		...
		sp.stop("Could not install superpowers skills (offline?). Run cheap update to retry.")
	}
