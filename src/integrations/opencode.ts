 ...
 * `["@cheap-dev/opencode-cheap"]` string list which loses the per-plugin
 * config block.
 */
 ...
 * `@cheap-dev/opencode-cheap@1.14.0` → `"1.14.0"`. Returns null for the
 * un-pinned form (`@cheap-dev/opencode-cheap`).
 */
...
interface PluginUpdateResult {
	...
	skippedCheapPlugin: boolean
}

/**
 * Pure transform on the plugin array: strip out any prior Cheap entries,
...
export function buildUpdatedPlugins(inputs: PluginUpdateInputs): PluginUpdateResult {
	...
	for (const entry of plugins) {
		if (Array.isArray(entry) && entry.length > 0 && typeof entry[0] === "string") {
			...
			// Legacy string format: "@cheap-dev/opencode-cheap" or "@cheap-dev/opencode-cheap@1.14.0"
	...
	if (!versionToPin) {
		return { plugins: filtered, skippedCheapPlugin: true }
	}

	const updatedEntry: unknown[] = [`${OPENCODE_PLUGIN_PACKAGE}@${versionToPin}`, pluginConfig]
	return { plugins: [...filtered, updatedEntry], skippedCheapPlugin: false }
}
