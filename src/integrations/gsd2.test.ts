...
import { buildGsd2CheapProvider, buildGsd2Preferences } from "./gsd2.js"
import { byId } from "./registry.js"

describe("buildGsd2CheapProvider", () => {
	it("returns the provider block with apiKey, baseUrl, and defaultModel", () => {
		const p = buildGsd2CheapProvider("test-key", TEST_MODELS) as {
			apiKey: string
			baseUrl: string
			defaultModel: string
		}
		expect(p.apiKey).toBe("test-key")
		expect(p.baseUrl).toBe("https://llm.cheap.dev/openai/v1")
		// defaultModel is the resolved main model (first serverless vision model).
		expect(p.defaultModel).toBe("kimi-k2.6")
	})

	it("emits all models with cost metadata (Opus/Sonnet billed at sticker, cheap models free)", () => {
		const p = buildGsd2CheapProvider("k", TEST_MODELS) as {
	...
	it("Kimi models accept text+image input, others text only", () => {
		const p = buildGsd2CheapProvider("k", TEST_MODELS) as { models: Array<{ id: string; input: string[] }> }
...
describe("buildGsd2Preferences", () => {
	...
	it("routes the high-level task buckets to the right model tiers", () => {
		...
		expect(prefs).toMatch(/planning: cheap\/(claude-opus-4-6|kimi-k2\.6)/)
		expect(prefs).toMatch(/validation: cheap\/(claude-opus-4-6|kimi-k2\.6)/)
		expect(prefs).toMatch(/auto_supervisor:\s*\n\s*model: cheap\/(claude-opus-4-6|kimi-k2\.6)/)
		// Coding model for research + subagent.
		expect(prefs).toContain("research: cheap/nemotron-3-ultra-fp4")
		expect(prefs).toContain("subagent: cheap/nemotron-3-ultra-fp4")
		// Sub model for execution_simple + discuss.
		expect(prefs).toContain("discuss: cheap/minimax-m2.7")
		expect(prefs).toContain("execution_simple: cheap/minimax-m2.7")
	})

	it("uses the new tier_models shape (light → coding, standard → sub, heavy → [main, planning])", () => {
		expect(prefs).toContain("light: cheap/nemotron-3-ultra-fp4")
		expect(prefs).toContain("standard: cheap/minimax-m2.7")
		expect(prefs).toMatch(/heavy:\s*\n\s*- cheap\/kimi-k2\.6\s*\n\s*- cheap\/(claude-opus-4-6|kimi-k2\.6)/)
	})
...
describe("gsd2 tool registration", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "cheap-gsd2-test-"))
	...
	it("write() emits both ~/.gsd/agent/models.json and ~/.gsd/preferences.md", async () => {
		...
		expect(models.providers.cheap.apiKey).toBe("secret-123")
	...
	it("write() preserves other providers and top-level keys in models.json", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				...
				providers: {
					...
					cheap: { apiKey: "stale-cheap-key", legacy: true },
				},
			}),
		)

		const tool = byId("gsd2")
		await tool?.write("global", "fresh-cheap-key", TEST_MODELS)
		...
		expect(models.providers.cheap.apiKey).toBe("fresh-cheap-key")
		expect(models.providers.cheap.legacy).toBeUndefined()
	})
})
