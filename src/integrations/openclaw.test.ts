...
describe("buildOpenClawProviderBlock", () => {
	it("uses the cheap base URL and ${CHEAP_API_KEY} placeholder", () => {
		...
		expect(block.baseUrl).toBe("https://llm.cheap.dev/openai/v1")
		expect(block.apiKey).toBe("${CHEAP_API_KEY}")
	...
	it("includes per-model id/name/contextWindow/maxTokens", () => {
		...
		const main = block.models.find((m) => m.id === "cheap/kimi-k2.6")
...
describe("buildOpenClawModelsCatalog", () => {
	it("maps each model id to its display alias", () => {
		...
		expect(catalog["cheap/kimi-k2.6"]).toEqual({ alias: "Kimi K2.6" })
		expect(catalog["cheap/kimi-k2.5"]).toEqual({ alias: "Kimi K2.5" })
		expect(catalog["cheap/nemotron-3-ultra-fp4"]).toEqual({ alias: "Nemotron 3 Ultra FP4" })
		expect(catalog["cheap/minimax-m2.7"]).toEqual({ alias: "MiniMax M2.7" })
	})
...
describe("writeOpenClawEnv", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "cheap-openclaw-test-"))
	...
	it("creates the .env file with the API key when none exists", () => {
		...
		expect(readFileSync(join(tmp, ".openclaw", ".env"), "utf-8")).toBe("CHEAP_API_KEY=test-key-123\n")
	})

	it("creates the parent directory if missing", () => {
		writeOpenClawEnv("fresh")
		expect(readFileSync(join(tmp, ".openclaw", ".env"), "utf-8")).toBe("CHEAP_API_KEY=fresh\n")
	})

	it("replaces an existing CHEAP_API_KEY line in place, preserving other entries", () => {
		mkdirSync(join(tmp, ".openclaw"), { recursive: true })
		writeFileSync(join(tmp, ".openclaw", ".env"), "OTHER_VAR=keep-me\nCHEAP_API_KEY=old-key\nALSO=keep\n", "utf-8")
		writeOpenClawEnv("new-key")
		expect(readFileSync(join(tmp, ".openclaw", ".env"), "utf-8")).toBe(
			"OTHER_VAR=keep-me\nCHEAP_API_KEY=new-key\nALSO=keep\n",
		)
	})

	it("appends CHEAP_API_KEY when the file exists but doesn't have one yet", () => {
		mkdirSync(join(tmp, ".openclaw"), { recursive: true })
		writeFileSync(join(tmp, ".openclaw", ".env"), "OTHER=foo\n", "utf-8")
		writeOpenClawEnv("appended")
		expect(readFileSync(join(tmp, ".openclaw", ".env"), "utf-8")).toBe("OTHER=foo\nCHEAP_API_KEY=appended\n")
	})
})
...
describe("writeOpenClawDirect integration", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "cheap-openclaw-direct-"))
		prevHome = process.env.HOME
		process.env.HOME = tmp
	...
	it("preserves existing user config keys like temperature and fallbacks", async () => {
		...
		expect(written.agents.defaults.model.fallbacks).toContain("cheap/nemotron-3-ultra-fp4")
		expect(written.agents.defaults.model.fallbacks).toContain("cheap/minimax-m2.7")
		expect(written.agents.defaults.models["other/model"]).toEqual({ alias: "Other" })
		expect(written.agents.defaults.models["cheap/kimi-k2.6"]).toBeDefined()
		expect(written.models.providers.cheap).toBeDefined()
	})
})
...
describe("writeOpenClawViaCLI integration", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "cheap-openclaw-cli-"))
		prevHome = process.env.HOME
		process.env.HOME = tmp
	...
	it("preserves existing fallbacks and other keys via CLI merge", async () => {
		...
		expect(mergedFallbacks).toContain("cheap/nemotron-3-ultra-fp4")
		...
		expect(mergedModels["cheap/kimi-k2.6"]).toBeDefined()
		expect(mergedModels["cheap/nemotron-3-ultra-fp4"]).toBeDefined()

		const envContent = readFileSync(join(tmp, ".openclaw", ".env"), "utf-8")
		expect(envContent).toBe("CHEAP_API_KEY=test-key-123\n")
	})
})
