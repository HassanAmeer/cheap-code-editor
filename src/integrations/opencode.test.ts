...
describe("extractVersionFromPluginName", () => {
	it("returns the suffix after @", () => {
		expect(extractVersionFromPluginName("@cheap-dev/opencode-cheap@1.14.0")).toBe("1.14.0")
		expect(extractVersionFromPluginName("@cheap-dev/opencode-cheap@latest")).toBe("latest")
	})
	it("returns null for the bare package name (no version pin)", () => {
		expect(extractVersionFromPluginName("@cheap-dev/opencode-cheap")).toBeNull()
	})
...
describe("buildUpdatedPlugins", () => {
	const CHEAP = "@cheap-dev/opencode-cheap"

	it("inserts a Cheap entry when none exists, using latestVersion", () => {
		const r = buildUpdatedPlugins({ plugins: [], telemetryEnabled: false, latestVersion: "1.14.0" })
		expect(r.skippedCheapPlugin).toBe(false)
		expect(r.plugins).toEqual([[`${CHEAP}@1.14.0`, {}]])
	})
	it("preserves unrelated plugins and replaces only the Cheap entry", () => {
		const existing = [
			["@other/plugin@1.0.0", { foo: "bar" }],
			[`${CHEAP}@1.10.0`, { stale: true }],
		]
	...
	it("falls back to the existing Cheap version when npm is unavailable", () => {
		const existing = [[`${CHEAP}@1.13.0`, {}]]
		const r = buildUpdatedPlugins({ plugins: existing, telemetryEnabled: false, latestVersion: null })
		expect(r.plugins).toEqual([[`${CHEAP}@1.13.0`, {}]])
		expect(r.skippedCheapPlugin).toBe(false)
	})
	it("skips the Cheap plugin when there's no version anywhere (npm + no prior entry)", () => {
		...
		expect(r.skippedCheapPlugin).toBe(true)
	})
	...
	it("strips the bare-package form (no version pin) like the versioned form", () => {
		const existing = [[CHEAP, { stale: true }]]
	...
	it("handles legacy string format (no version pin) and re-adds with pinned version", () => {
		const existing = ["@other/plugin", CHEAP]
		const r = buildUpdatedPlugins({ plugins: existing, telemetryEnabled: false, latestVersion: "1.14.0" })
		expect(r.plugins).toEqual(["@other/plugin", [`${CHEAP}@1.14.0`, {}]])
		expect(r.skippedCheapPlugin).toBe(false)
	})
	it("handles legacy string format with existing version and falls back to it when npm unavailable", () => {
		const existing = ["@other/plugin", `${CHEAP}@1.13.0`]
		const r = buildUpdatedPlugins({ plugins: existing, telemetryEnabled: false, latestVersion: null })
		expect(r.plugins).toEqual(["@other/plugin", [`${CHEAP}@1.13.0`, {}]])
		expect(r.skippedCheapPlugin).toBe(false)
	})
	it("skips legacy string Cheap plugin when no version anywhere (npm + no prior entry)", () => {
		const r = buildUpdatedPlugins({ plugins: ["@other/plugin", CHEAP], telemetryEnabled: false, latestVersion: null })
...
describe("opencode tool registration", () => {
	...
	beforeEach(() => {
		scratchHome = mkdtempSync(join(tmpdir(), "cheap-opencode-test-"))
	...
	it("write() merges provider + plugin into opencode.json without clobbering unrelated keys", async () => {
		...
		try {
			...
			expect(written.provider.cheap).toBeDefined()
			expect((written.provider.cheap as { options: { apiKey: string } }).options.apiKey).toBe("test-key-123")
			expect(written.model).toBe("cheap/kimi-k2.6")
