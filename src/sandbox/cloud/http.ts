...
export function resolveEndpoint(options?: AuthenticateOptions): string {
	const fromEnv = process.env.CHEAP_REMOTE_ENDPOINT
	return options?.endpoint ?? fromEnv ?? "https://app.cheap.dev/api"
}
...
export async function checkResponse(resp: Response, endpoint: string): Promise<void> {
	...
	switch (resp.status) {
		case 401:
			throw new RemoteAuthError(`Invalid API key - run 'cheap setup' to authenticate: ${endpoint}`, 401)
		case 403:
