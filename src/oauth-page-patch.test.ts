...
// The Cheap logo SVG in both templates uses this exact orange — distinct from
...
const CHEAP_LOGO_ORANGE = "#FF521D"
...
beforeEach(() => {
	originalTemplateDir = process.env.CHEAP_OAUTH_TEMPLATE_DIR
	process.env.CHEAP_OAUTH_TEMPLATE_DIR = templateDir
})
...
afterEach(() => {
	if (originalTemplateDir === undefined) {
		...
		delete process.env.CHEAP_OAUTH_TEMPLATE_DIR
	} else {
		process.env.CHEAP_OAUTH_TEMPLATE_DIR = originalTemplateDir
	}
})

it("renders the Cheap-branded success page with substitutions", async () => {
	const { oauthSuccessHtml } = await import(/* @vite-ignore */ patchedFileUrl)

	const html = oauthSuccessHtml("You can close this window.")

	expect(html).toContain(CHEAP_LOGO_ORANGE)
	expect(html).toContain("You can close this window.")
	expect(html).not.toMatch(/\{\{[A-Z_]+\}\}/)
	expect(html).not.toContain('class="details"')
})

it("renders the Cheap-branded error page with escaped details", async () => {
...
it("falls back to a minimal unbranded page when the template dir is unreadable", async () => {
	process.env.CHEAP_OAUTH_TEMPLATE_DIR = resolve(projectRoot, "this/path/does/not/exist")
	const { oauthSuccessHtml } = await import(/* @vite-ignore */ patchedFileUrl)
	...
	// Fallback must not silently misbrand as Cheap (orange marker) or Pi
	// (the upstream LOGO_SVG path that the original fallback shipped).
	expect(html).not.toContain(CHEAP_LOGO_ORANGE)
