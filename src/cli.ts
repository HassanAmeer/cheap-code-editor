...
import "./model-selector-patch.js"
...
import cheapMinimalTintsExtension from "./extensions/cheap-minimal-tints.js"
...
import { writeCheapKeybindingDefaults } from "./extensions/permissions/keybindings.js"
...
try {
	// Top-level cheap subcommands (setup, claude, opencode, …) and the
	...
	// the version using piConfig.name = "cheap".
	...
	if (helpOrVersion) {
		...
		const envKey = process.env.CHEAP_API_KEY || undefined
		...
		if (!agentDir) {
			throw new Error("CHEAP_CODING_AGENT_DIR is not set; cli.ts must be entered via entry.ts")
		}
		...
		// override in place.
		writeCheapKeybindingDefaults(agentDir)
		...
		try {
			...
			if ((err as NodeJS.ErrnoException).code === "ENOENT") {
				writeFileSync(settingsPath, `${JSON.stringify({ quietStartup: true, theme: "cheap-minimal" }, null, 2)}\n`)
			} else {
		...
		// wanting custom colors should clone+rename (e.g. `my-cheap.json`), which
		// this loop won't touch. The cheap-minimal source keeps all six bg tokens
		// as `""` placeholders; the cheap-minimal-tints extension fills them in
		...
		// the cheap-minimal-tints and terminal-colors extensions. Skip non-TUI
		...
		// this loop injected per-process bg tints into cheap-minimal.json on every
		// startup; that race is what made terminals clobber each other's colors.
		// Tints now live in memory only, applied by the cheap-minimal-tints
		...
		// Clear the visible viewport and home the cursor so cheap renders at the top.
		if (terminalStartupOutputAllowed) {
		...
		const fetchPatchedSymbol = Symbol.for("cheap.fetchPatched")
		if (!(globalThis.fetch as typeof globalThis.fetch & { [key: symbol]: boolean })[fetchPatchedSymbol]) {
			const userAgent = `${process.env.COMMAND_NAME || "cheap"}/${getVersion()}`
		...
		const terminalUiExtensionFactories = isTerminalUiMode(rawArgs, terminalIo)
			? [terminalColorsExtension, cheapMinimalTintsExtension, uiExtension]
			: []
		...
		if (acpMode) {
			...
			// Delegate to pi-mono's CLI main function, injecting the cheap extension
			const { main } = await import("@earendil-works/pi-coding-agent")
