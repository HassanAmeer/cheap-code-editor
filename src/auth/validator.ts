...
/**
 * Probe the Cast AI / Cheap API with the given key. 200 = valid, 401 =
...
export async function validateApiKey(apiKey: string, options: ValidatorOptions = {}): Promise<ValidateResult> {
	if (apiKey === "") {
		return {
			...
			suggestions: [
				"Get your API key at https://app.cheap.dev",
				"Set it via CHEAP_API_KEY environment variable or run 'cheap setup'",
			],
		}
	}
	...
	try {
		...
		return {
			valid: false,
			error: "Network error: unable to reach Cheap API",
			suggestions: [
	...
	if (resp.status === 401) {
		return {
			...
			suggestions: [
				"Verify your API key at https://app.cheap.dev",
	...
	if (resp.status === 403) {
		return {
			...
			suggestions: [
				"Verify your API key has the required scopes at https://app.cheap.dev",
