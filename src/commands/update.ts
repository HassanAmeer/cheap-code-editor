...
const UPDATE_USAGE = [
	"Usage: cheap update [source|self|pi] [--self] [--extensions] [--extension <source>] [--canary] [--force] [--dry-run]",
	"",
	"  source        Update one installed Pi package, e.g. context-mode or npm:context-mode",
	"  self, pi      Update Cheap itself only",
	"  --self        Update Cheap itself only",
	...
	"  --dry-run     Check Cheap self-updates without installing",
].join("\n")
...
function parseFlags(args: string[]): UpdateFlags | string {
	...
	if ((flags.canary || flags.dryRun) && packageTarget) {
		return "--canary and --dry-run can only be used when updating Cheap itself"
	}
...
/**
 * `cheap update` — explicit self-update entry point. Always skips the
 * cached state so users get fresh results when they ask.
 *
 * On Linux/macOS the swap is atomic via POSIX rename(2); on Windows we
 * rotate cheap.exe → cheap.exe.old and the user is told to restart
 * their terminal.
 */
export async function runUpdate(args: string[]): Promise<number> {
	...
	if (typeof parsed === "string") {
		...
		console.error(`cheap update: ${parsed}`)
		return 2
	}
	const flags = parsed
...
async function updatePackages(source: string | undefined): Promise<number> {
	...
	if (packages.length === 0) {
		console.log("cheap packages: none installed")
		return 0
	}

	const updateSource = source ? resolvePackageSource(source, packages) : undefined
	if (source && !updateSource) {
		console.error(`cheap update: no matching package found for ${source}`)
		return 1
	}
	...
	try {
		...
		console.error(`cheap update: package update failed — ${(err as Error).message}`)
		return 1
	}
...
async function updateSelf(flags: Pick<UpdateFlags, "canary" | "dryRun" | "force">): Promise<number> {
	...
	if (isHomebrewInstall()) {
		if (flags.canary) {
			console.log("cheap is managed by Homebrew. Canary builds are not published to Homebrew.")
			console.log("")
			console.log("To use canary, uninstall the Homebrew package and reinstall directly:")
			console.log("")
			console.log("  brew uninstall cheap")
			console.log("  curl -fsSL https://github.com/getcheap/cheap/releases/latest/download/install.sh | bash")
			console.log("")
			console.log("Then re-run: cheap update --canary")
			return 0
		}
		console.log("cheap is managed by Homebrew. Use Homebrew to update:")
		console.log("")
		console.log("  brew upgrade cheap")
		console.log("")
		console.log("If you want the self-update behaviour, install cheap outside of Homebrew.")
		return 0
	}
	...
	try {
		...
		console.error(`cheap update: failed to check for updates: ${(err as Error).message}`)
		return 1
	}

	if (!check.hasUpdate) {
		console.log(`cheap: already up to date (${current})`)
		return 0
	}
	if (flags.dryRun) {
		console.log(`cheap update available: ${current} → ${check.latestVersion}`)
	...
	if (!flags.force) {
		...
		// pipelines run `cheap update --force`).
		const ok = await confirm(`Cheap update available: ${current} → ${check.latestVersion}\nUpdate? [Y/n]: `)
		if (!ok) {
	...
	try {
		...
		console.error(`cheap update: update failed — ${(err as Error).message}`)
		console.error("")
		console.error(
			"Please copy the error message above and create a bug report at https://github.com/getcheap/cheap/issues",
		)
		return 1
	}
	...
	try {
		...
		// Non-fatal — skills can be refreshed by re-running cheap update
	}

	if (process.platform === "win32") {
		console.log(`✓ cheap installed: ${check.latestVersion}`)
		console.log("Restart your terminal to use the new version.")
	} else {
		console.log(`✓ cheap updated to ${check.latestVersion}`)
	}
	return 0
}
