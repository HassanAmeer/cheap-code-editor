...
import { runConfigModel } from "./config-model.js"

const TELEMETRY_ENV = "CHEAP_TELEMETRY_ENABLED"

/**
 * `cheap config telemetry [on|off]` — show or set telemetry.enabled in
 * config.json. With no value, prints the current state (and notes when
 * an env-var override is in effect).
 *
 * Anything else under `cheap config <subcommand>` is unrecognised; we
...
export async function runConfig(args: string[]): Promise<number> {
	...
	switch (sub) {
		...
		default:
			console.error(`cheap config: unknown subcommand "${sub}"`)
			printUsage()
			return 2
	}
}
...
function handleTelemetry(args: string[]): number {
	...
	if (enabled === null) {
		console.error(`cheap config telemetry: expected "on" or "off", got "${args[0]}"`)
		return 2
	}
...
function printUsage(): void {
	console.error("Usage: cheap config telemetry [on|off]")
	console.error("       cheap config telemetry           # show current status")
	console.error("       cheap config model               # configure AI providers & custom models")
}
