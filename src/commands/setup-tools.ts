...
/**
 * `cheap setup-tools` — interactive wizard to configure multiple coding
 * tools (Cursor, OpenCode, Claude Code, OpenClaw, GSD2) in one pass.
 *
 * This command extracts the "tools" step that used to be part of
 * `cheap setup`, so users can re-run tool configuration without going
...
export async function runSetupTools(args: string[]): Promise<number> {
	...
	if (!apiKey) {
		console.error("cheap: no API key configured. Run `cheap setup` or set $CHEAP_API_KEY.")
		return 1
	}

	intro("cheap setup-tools")
	...
	// `cheap setup`, `cheap config telemetry`, or $CHEAP_TELEMETRY_ENABLED)
