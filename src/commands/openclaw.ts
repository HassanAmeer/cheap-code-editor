...
export async function runOpenClaw(args: string[]): Promise<number> {
	...
	try {
		...
		if (!tool) {
			console.error("cheap openclaw: integration not registered")
			return 1
		}
		await tool.write(scope, prepped.apiKey, prepped.models)
		console.log("cheap openclaw: configuration written.")
		return 0
	} catch (err) {
		console.error(`cheap openclaw: ${(err as Error).message}`)
		return 1
	}
}
