...
describe("runUpdate flag parsing", () => {
	...
	it("--help documents --canary", async () => {
		...
		expect(out).toContain("Usage: cheap update")
	})
	...
	it("rejects global Pi flags that are not valid for update", async () => {
		const code = await runUpdate(["--model", "cheap-dev/kimi-k2.6"])
		expect(code).toBe(2)
...
describe("runUpdate Homebrew branch", () => {
	...
	it("prints canary-specific message on Homebrew + --canary and skips download", async () => {
		...
		expect(out).toContain("brew uninstall cheap")
		expect(out).toContain("install.sh")
		expect(out).toContain("cheap update --canary")
		expect(out).not.toContain("brew upgrade cheap")
	...
	it("prints generic Homebrew message on bare update (no --canary)", async () => {
		...
		expect(code).toBe(0)
		const out = logSpy.mock.calls.map((c) => String(c[0] ?? "")).join("\n")
		expect(out).toContain("brew upgrade cheap")
		expect(out).not.toContain("brew uninstall cheap")
		expect(checkForUpdateMock).not.toHaveBeenCalled()
	})
})
...
describe("runUpdate package targets", () => {
	...
	it("updates all packages before checking Cheap self-updates on bare update", async () => {
		const code = await runUpdate([])
		...
		expect(out).toContain("cheap: already up to date")
	})
	...
	it("keeps --dry-run as a Cheap self-update check", async () => {
