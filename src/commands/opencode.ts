...
/**
 * `cheap opencode [args]` — write the cheap provider into the user's
 * opencode.json (override mode), then launch opencode. Future runs see
 * the cheap provider without going through us.
 *
...
export async function runOpenCode(args: string[]): Promise<number> {
	...
	try {
		...
		if (!tool) {
			console.error("cheap opencode: integration not registered")
			return 1
		}
		await tool.write(scope, prepped.apiKey, prepped.models)
		return await runForeground("opencode", args)
	} catch (err) {
		console.error(`cheap opencode: ${(err as Error).message}`)
		return 1
	}
}
