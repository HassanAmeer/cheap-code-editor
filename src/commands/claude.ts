...
/**
 * `cheap claude [args]` — launch Claude Code with cheap env vars injected
 * for this run only (no disk write). The user's ~/.claude/settings.json is
 * left untouched; persistent install lives behind `cheap setup` (or a
 * future `cheap claude --persist`).
 *
 * All args after `claude` are forwarded to the binary verbatim — that's how
 * `cheap claude --help`, `cheap claude /resume`, etc. work without us
...
export async function runClaude(args: string[]): Promise<number> {
	...
	try {
		...
		console.error(`cheap claude: ${(err as Error).message}`)
		return 1
	}
}
