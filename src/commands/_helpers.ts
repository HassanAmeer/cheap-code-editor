...
/**
 * Resolve the cheap API key from $CHEAP_API_KEY first, then the config
 * file. Returns null when neither is set; callers print a friendly "run
 * cheap setup" message rather than throwing.
 */
export function resolveApiKey(): string | null {
	const env = process.env.CHEAP_API_KEY
...
export async function prepareTool(
	...
	if (!tool) {
		...
		console.error(`cheap: integration "${toolId}" not registered.`)
		return null
	}
