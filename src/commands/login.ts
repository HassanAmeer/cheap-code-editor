...
export async function runLogin(_args: string[]): Promise<number> {
	...
	const keyExport = exportEnvToShellProfile(CHEAP_API_KEY_ENV, token)
	if (keyExport.path) {
		console.log(`${CHEAP_API_KEY_ENV} exported to ${keyExport.path}`)
	} else if (keyExport.error) {
		console.warn(`Could not export ${CHEAP_API_KEY_ENV} to shell profile: ${keyExport.error}`)
	}
...
export function getLoginHelp(): string {
	const lines: string[] = [
		"Open the browser to log in to Cheap via our web app and generate an API key.",
