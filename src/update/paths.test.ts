...
describe("isHomebrewInstall", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "cheap-paths-test-"))
	...
	it("returns false on win32 regardless of path", () => {
		setPlatform("win32")
		setExecPath("C:\\Program Files\\cheap\\cheap.exe")
		expect(isHomebrewInstall()).toBe(false)
	})

	it("returns true via symlink detection (bin → Cellar)", () => {
		// Fake layout:
		//   <tmp>/Cellar/cheap/1.2.3/bin/cheap   ← real binary
		//   <tmp>/bin/cheap                        ← symlink → Cellar path
		const cellarBin = join(tmp, "Cellar", "cheap", "1.2.3", "bin")
		const realBinary = join(cellarBin, "cheap")
		touch(realBinary)

		const prefixBin = join(tmp, "bin")
		mkdirSync(prefixBin, { recursive: true })
		const symlink = join(prefixBin, "cheap")
	...
	it("returns false when a real (non-symlink) binary sits in <prefix>/bin/", () => {
		...
		//   <tmp>/bin/cheap         ← real file dropped by install.sh, NOT a symlink
		...
		const binPath = join(tmp, "bin", "cheap")
		touch(binPath)
	...
	it("returns false when Cellar directory does not exist under prefix", () => {
		...
		setExecPath(join(fakePrefix, "bin", "cheap"))
		expect(isHomebrewInstall()).toBe(false)
	})
})
