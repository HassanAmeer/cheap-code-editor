...
const REPO: Repo = { owner: "castai", name: "cheap-dev", binary: "cheap" }
...
describe("checkForUpdate version comparison", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "cheap-workflow-test-"))
...
describe("checkForUpdate canary path", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "cheap-workflow-canary-test-"))
		prevHome = process.env.HOME
		prevXdg = process.env.XDG_CACHE_HOME
		process.env.XDG_CACHE_HOME = tmp
	})
...
describe("applyUpdate share destination", () => {
	...
	beforeEach(() => {
		...
		fakePrefix = mkdtempSync(join(tmpdir(), "cheap-prefix-test-"))
		fakeBinPath = join(fakePrefix, "bin", "cheap")
		mkdirSync(join(fakePrefix, "bin"), { recursive: true })
		writeFileSync(fakeBinPath, "")

		// Create extracted archive root.
		extractRoot = mkdtempSync(join(tmpdir(), "cheap-extract-test-"))
		mkdirSync(join(extractRoot, "bin"), { recursive: true })
		mkdirSync(join(extractRoot, "share", "cheap"), { recursive: true })
		writeFileSync(join(extractRoot, "bin", "cheap"), "")
		writeFileSync(join(extractRoot, "share", "cheap", "package.json"), "{}")
	...
	it("writes share files to the sibling share when it exists", async () => {
		...
		const siblingShare = join(fakePrefix, "share", "cheap")
		...
		expect(mocks.copySupportingFiles).toHaveBeenCalledWith(join(extractRoot, "share", "cheap"), siblingShare, "cheap")
	})

	it("falls back to ~/.local/share/cheap when the sibling share does not exist", async () => {
		const client = {
			fetchChecksum: mocks.fetchChecksum,
			downloadArchive: mocks.downloadArchive,
		} as unknown as GitHubClient
		...
		expect(mocks.copySupportingFiles).toHaveBeenCalledWith(
			join(extractRoot, "share", "cheap"),
			expect.stringContaining(".local/share/cheap"),
			"cheap",
		)
	})
})
