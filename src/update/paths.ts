...
/** State file path: ~/.cache/cheap/state.json (or under $XDG_CACHE_HOME). */
export function statePath(): string {
	return join(cacheDir(), APP_DIR, "state.json")
}

/** Backups dir: ~/.cache/cheap/backups/. Created on demand by the installer. */
export function backupDir(): string {
	return join(cacheDir(), APP_DIR, "backups")
}

/**
 * The real path of the running cheap binary, with symlinks resolved. We
 * follow symlinks so writing to the result lands on the actual file
 * (Homebrew, manual installs, etc. all symlink cheap from /usr/local/bin).
 */
export function resolveExecutablePath(): string {
	// process.execPath is the running binary in a Bun-compiled `cheap` —
...
/**
 * Data directory for supporting files (share/cheap contents), honoring
 * XDG_DATA_HOME on Linux. Falls back to ~/.local/share/cheap.
 */
