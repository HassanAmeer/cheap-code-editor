...
import { CHEAP_REPO, GitHubClient, type Repo } from "./github.js"
 ...
 * $CHEAP_NO_UPDATE_CHECK by short-circuiting to a "no update" result.
 *
...
export async function checkForUpdate(opts: CheckOptions): Promise<CheckResult> {
	const repo = opts.repo ?? CHEAP_REPO
 ...
 *
 * On Windows the install rotates `cheap.exe` → `cheap.exe.old` and drops
...
export async function applyUpdate(opts: UpdateOptions): Promise<UpdateResult> {
	...
	const workDir = mkdtempSync(join(tmpdir(), "cheap-update-"))
	try {
		...
		try {
			...
			const shareSrc = join(extractedRoot, "share", "cheap")
