...
describe("copySupportingFiles", () => {
	...
	beforeEach(() => {
		srcDir = mkdtempSync(join(tmpdir(), "cheap-install-src-"))
		dstDir = mkdtempSync(join(tmpdir(), "cheap-install-dst-"))
	})
	...
	it("skips entries matching skipName", () => {
		writeFileSync(join(srcDir, "cheap"), "binary")
		writeFileSync(join(srcDir, "keepme.txt"), "ok")

		copySupportingFiles(srcDir, dstDir, "cheap")

		expect(readFileSync(join(dstDir, "keepme.txt"), "utf-8")).toBe("ok")
		expect(() => readFileSync(join(dstDir, "cheap"))).toThrow()
	})
})
...
describe("atomicInstall", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "cheap-atomic-install-test-"))
	...
	it("same-fs rename: replaces current binary and creates backup on POSIX", () => {
		setPlatform("linux")
		const currentPath = join(tmp, "bin", "cheap")
		const newPath = join(tmp, "new", "cheap")
