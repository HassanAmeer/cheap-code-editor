...
// Minimal fake of AgentSession surface used by CheapAcpAgent. The factory seam
...
describe("CheapAcpAgent turn lifecycle", () => {
	...
	describe("initialize capability detection", () => {
		const tempAgentDir = "/tmp/cheap-acp-test-agent-dir"
		...
		it("declares image: true when cached models include vision models", async () => {
			...
			try {
				...
				const testAgent = new CheapAcpAgent(makeConn(), {
					extensionFactories: [],
	...
	it("drops stray tool_execution_start/end after a short-circuited turn", async () => {
		...
		const localAgent = new CheapAcpAgent(conn, {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
	...
	it("disposes the session if subscribe throws during newSession", async () => {
		...
		const localAgent = new CheapAcpAgent(makeConn(), {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory: factory,
		})
	...
	// mcpServers is declared in the ACP request shape but cheap has no hook to
	...
	it("isolates turn state across parallel sessions", async () => {
		...
		const parallelAgent = new CheapAcpAgent(makeConn(), {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
...
describe("CheapAcpAgent tool execution stream", () => {
	let fake: FakeAgentSession
	let agent: CheapAcpAgent
	let sessionId: string
	...
	beforeEach(async () => {
		...
		updates = rec.updates
		agent = new CheapAcpAgent(rec.conn, {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory,
		})
...
describe("newSession model state", () => {
	it("returns model state in the response when a model is available", async () => {
		...
		const agent = new CheapAcpAgent(makeConn(), {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory: factory,
		})
...
describe("setSessionConfigOption", () => {
	...
	it("emits config_option_update when permission mode changes", async () => {
		...
		const sessionFactory: AcpSessionFactory = async () => asSession(fake)
		const agent = new CheapAcpAgent(conn, {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory,
		})
...
describe("session mode controller lifecycle", () => {
	...
	it("defaults to 'default' mode when CHEAP_PERMISSIONS env is unset", async () => {
		vi.stubEnv(PERMISSIONS_ENV_KEY, "")
		Reflect.deleteProperty(process.env, PERMISSIONS_ENV_KEY)
		const agent = new CheapAcpAgent(makeConn(), {
			extensionFactories: [],
	...
	it("uses defaultMode from permissions config file when env is not set", async () => {
		// Create a temp directory with a .cheap/permissions.json
		const tmpDir = mkdtempSync(join(tmpdir(), "acp-mode-config-test-"))
		const cheapDir = join(tmpDir, ".cheap")
		mkdirSync(cheapDir, { recursive: true })
		writeFileSync(join(cheapDir, "permissions.json"), JSON.stringify({ defaultMode: "plan" }))
	...
	it("env CHEAP_PERMISSIONS takes precedence over config defaultMode", async () => {
		// Create a temp directory with a .cheap/permissions.json
	...
	it("closeSession deletes the CHEAP_PERMISSIONS_<sessionId> env key", async () => {
		vi.stubEnv(PERMISSIONS_ENV_KEY, "")
		Reflect.deleteProperty(process.env, PERMISSIONS_ENV_KEY)
	...
	it("shutdown deletes CHEAP_PERMISSIONS_<sessionId> env keys for all sessions", async () => {
		vi.stubEnv(PERMISSIONS_ENV_KEY, "")
		Reflect.deleteProperty(process.env, PERMISSIONS_ENV_KEY)
...
describe("CheapAcpAgent listSessions", () => {
	function makeAgent(lister: AcpSessionLister): CheapAcpAgent {
		return new CheapAcpAgent(makeConn(), {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
...
describe("CheapAcpAgent loadSession", () => {
	function makeAgent(loader: AcpSessionLoader, opts?: { conn?: AgentSideConnection }): CheapAcpAgent {
		return new CheapAcpAgent(opts?.conn ?? makeConn(), {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory: async () => asSession(new FakeAgentSession("unused")),
	...
	it("returns configOptions in loadSession response", async () => {
		const tmpDir = mkdtempSync(join(tmpdir(), "cheap-acp-load-config-"))
		try {
	...
	it("rejects default-loaded sessions whose header cwd disagrees before opening", async () => {
		const agentDir = mkdtempSync(join(tmpdir(), "cheap-acp-load-"))
		try {
