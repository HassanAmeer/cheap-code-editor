...
// @agentclientprotocol/sdk. Lets Zed / openclaw drive cheap in-process.
...
export class CheapAcpAgent implements Agent {
	 ...
	 * 1. Environment variable CHEAP_PERMISSIONS
	...
	async newSession(params: NewSessionRequest): Promise<NewSessionResponse> {
		// mcpServers isn't plumbed: cheap loads MCP servers from its own config via
		...
		if (Array.isArray(params.mcpServers) && params.mcpServers.length > 0) {
			throw RequestError.invalidParams(undefined, "mcpServers is not supported; configure MCP servers via cheap config")
		}
...
// CHEAP_API_KEY before we ever spawned the ACP loop, and updateModelsConfig
...
// listSessions points at cheap's agentDir (~/.config/cheap/harness/sessions/...)
// instead of pi's own ~/.pi/agent/sessions/... — pi reads PI_CODING_AGENT_DIR,
// not CHEAP_CODING_AGENT_DIR, so without explicit sessionDir threading the
// default lookup misses every cheap session. Encoding is a public on-disk
...
function defaultSessionLoader(options: RunAcpOptions): AcpSessionLoader {
	return async (params: LoadSessionRequest): Promise<AgentSession> => {
		...
		// not CHEAP_CODING_AGENT_DIR, so default lookups would miss cheap
		// sessions stored under the cheap agent dir.
...
// Mirrors the tool names cheap actually exposes: pi-coding-agent core tools
// plus the cheap extensions in src/extensions (web-fetch, web-search, Agent).
...
export async function runAcpMode(options: RunAcpOptions): Promise<void> {
	...
	let agentInstance: CheapAcpAgent | undefined
	const conn = new AgentSideConnection((c: AgentSideConnection) => {
		agentInstance = new CheapAcpAgent(c, options)
