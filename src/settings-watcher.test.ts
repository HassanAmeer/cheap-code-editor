...
beforeEach(() => {
	process.env.CHEAP_CODING_AGENT_DIR = "/fake/agent/dir"
	vi.useFakeTimers()
})
...
describe("getActiveThemeName", () => {
	...
	it("returns undefined when CHEAP_CODING_AGENT_DIR is unset", () => {
		// biome-ignore lint/performance/noDelete: process.env requires delete operator to be truly unset rather than stringified to "undefined"
		delete process.env.CHEAP_CODING_AGENT_DIR
		expect(getActiveThemeName()).toBeUndefined()
	})
...
describe("onThemeChange", () => {
	it("does not fire listener when theme has not changed", () => {
		mockReadFileSync.mockReturnValue(JSON.stringify({ theme: "cheap-minimal" }))
		const listener = vi.fn()

		// Subscribe — watcher captures lastSeenTheme = "cheap-minimal"
	...
	it("fires listener when theme changes", () => {
		mockReadFileSync
			.mockReturnValueOnce(JSON.stringify({ theme: "cheap-minimal" })) // ensureWatcher init read
		...
		expect(listener).toHaveBeenCalledWith("dark", "cheap-minimal")
		unsub()
	})
