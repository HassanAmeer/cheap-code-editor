...
describe("isInSandboxCluster", () => {
	...
	it("returns true when CHEAP_SANDBOX=1", () => {
		vi.stubEnv("CHEAP_SANDBOX", "1")
		expect(isInSandboxCluster()).toBe(true)
	})

	it("returns true when CHEAP_SANDBOX=true (case-insensitive)", () => {
		vi.stubEnv("CHEAP_SANDBOX", "true")
		expect(isInSandboxCluster()).toBe(true)
	})

	it("returns false when CHEAP_SANDBOX is not a truthy value", () => {
		vi.stubEnv("CHEAP_SANDBOX", "0")
	...
	it("prioritises CHEAP_SANDBOX over security-fallback signals", () => {
		vi.stubEnv("CHEAP_SANDBOX", "1")
		expect(isInSandboxCluster()).toBe(true)
	})
