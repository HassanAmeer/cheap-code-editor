// Apply per-process dynamic background tints for the `cheap-minimal` theme.
//
// We mutate the loaded Theme's bgColors Map directly instead of writing tints
// into the on-disk theme file. Why: when cheap runs simultaneously in two
...
export default function cheapMinimalTintsExtension(pi: ExtensionAPI) {
	...
	const triggerRerender = (ctx: ExtensionContext) => {
		...
		ctx.ui.setStatus("cheap-tints-rerender", undefined)
	}
	...
	pi.on("session_start", (_event, ctx: ExtensionContext) => {
		if (getActiveThemeName() === "cheap-minimal") applyTints(ctx)

		unsubscribeThemeChange?.()
		unsubscribeThemeChange = onThemeChange((newName) => {
			if (newName === "cheap-minimal") {
				applyTints(ctx)
				triggerRerender(ctx)
			}
		})
