...
export async function configureLocalProviderKeys(ctx: ExtensionContext, modelsJsonPath: string): Promise<void> {
	const providers = [
		...
		{ label: "Gemini (Google Studio)", envKey: "GEMINI_API_KEY", id: "gemini", defaultUrl: "https://generativelanguage.googleapis.com/v1beta/openai/" },
		{ label: "OpenRouter (Multi-model)", envKey: "OPENROUTER_API_KEY", id: "openrouter", defaultUrl: "https://openrouter.ai/api/v1" },
		...
		{ label: "Qwen", envKey: "QWEN_API_KEY", id: "qwen", defaultUrl: "https://dashscope.aliyuncs.com/compatible-mode/v1" },
		...
		if (action === "✦ Configure a Provider API Key" || action === "🗑️ Clear a Provider API Key") {
			...
			const selectedLabel = await ctx.ui.custom<string | undefined>((_tui, _theme, _keybindings, done) => {
				...
				const labels = [
					providers[0].label,
					"← Back",
					...providers.slice(1).map((p) => p.label)
				]
				const selector = new ExtensionSelectorComponent(
			...
			if (!isClear) {
				...
				const promptedKey = await ctx.ui.custom<string | undefined>((tui, _theme, _keybindings, done) => {
					const dialog = new LoginDialogComponent(tui as any, provider.id, () => done(undefined), provider.label, `Configure ${provider.label} Key`)
					dialog.showPrompt(`API Key (current: ${existingKey ? censorKey(existingKey) : "none"}):`, `API Key`)
						.then((val) => done(val))
			...
			ctx.ui.notify(isClear ? `${provider.label} configuration cleared.` : `${provider.label} configuration updated.`, "info")
		}
