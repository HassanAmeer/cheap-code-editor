...
import { spawnCheapSubprocess } from "../../utils/spawn-cheap-subprocess.js"
...
export function buildCuratorPrompt(candidates: CuratorCandidate[]): string {
	...
	return `You are the Cheap skill curator. Your job is **consolidation only** — not gap-finding, not creating new skills from scratch.
...
function spawnReviewAgent(
	...
	return spawnCheapSubprocess({
		args,
		stdout: opts?.stdout,
		stderr: opts?.stderr,
		detached: opts?.detached,
		env: { CHEAP_SUBAGENT: "1", CHEAP_SESSION_REVIEW: "1" },
	})
}
...
export function debugLog(msg: string): void {
	const path = process.env.CHEAP_REVIEW_LOG
	if (!path) return
...
export function spawnSessionReview(opts: RunSessionReviewOptions): void {
	...
	const reviewLogPath = process.env.CHEAP_REVIEW_LOG
