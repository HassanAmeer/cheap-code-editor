...
describe("handleMessageEnd", () => {
	...
	it("resolves provider via cheap-dev to ai-enabler", async () => {
		const ctx = makeCtx()
		const emitSpy = vi.spyOn(ctx, "emit")
...
describe("handleBeforeAgentStart", () => {
	...
	it("emits cheap.user_message with message_length and model", () => {
		const ctx = makeCtx()
		ctx.currentModel = "claude-3-5-sonnet"
		const emitSpy = vi.spyOn(ctx, "emit")
...
describe("handleAgentEnd", () => {
	...
	it("emits cheap.error when last message is a toolResult with isError=true", () => {
		const ctx = makeCtx()
		ctx.currentModel = "claude-3-5-sonnet"
		const emitSpy = vi.spyOn(ctx, "emit")
