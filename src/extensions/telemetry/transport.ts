...
const DEBUG_FLAG = process.env.CHEAP_TELEMETRY_DEBUG

function logEventToFile(event: string, properties: Record<string, unknown>): void {
	if (!DEBUG_FLAG) return
	try {
		const logDir = path.join(process.cwd(), ".cheap")
...
function resourceAttributes() {
	return [strAttr("service.name", "cheap"), strAttr("user_agent.original", `cheap/${getVersion()}`)]
}
