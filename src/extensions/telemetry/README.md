...
This extension collects usage data for the Cheap CLI and sends it via OTLP (OpenTelemetry Protocol).
...
| Resource attributes | `service.name="cheap"`, `user_agent.original="cheap/{version}"` |
| Scope | `name="cheap"`, `version="1.0.0"` |
...
- Telemetry is on by default and controlled by `telemetry.enabled` in `~/.config/cheap/config.json` (overridable via `$CHEAP_TELEMETRY_ENABLED`).
