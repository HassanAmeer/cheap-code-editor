...
// - Global:  ~/.config/cheap/harness/agents.json (via getAgentDir()) — manual defaults, never written here
// - Project: <cwd>/.cheap/agents.json — written by /agents → Settings; overrides global on load
...
function projectPath(cwd: string): string {
	return join(cwd, ".cheap", "agents.json")
}
...
function readSettingsFile(path: string): SubagentsSettings {
	...
	try {
		...
		console.warn(`[cheap-agents] Ignoring malformed settings at ${path}: ${reason}`)
		return {}
	}
}
