...
import { CHEAP_DEV_PROVIDER, MODEL_CAPABILITIES } from "../orchestration/model-registry/index.js"
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		// .cheap/agents/ directory and forgets the previous one.
	...
	const buildTypeListText = () => {
		...
		return [
			...
			"",
			`Custom agents can be defined in .cheap/agents/<name>.md (project) or ${getAgentDir()}/agents/<name>.md (global) — they are picked up automatically. Project-level agents override global ones. Creating a .md file with the same name as a default agent overrides it.`,
...
- Use standard-tier models for well-scoped tasks (CRUD, straightforward tests, mechanical fixes). Use heavy-tier models for complex concurrency, algorithms, or architectural reasoning. Use light-tier models for simple exploration or verification.`,
			parameters: Type.Object({
				...
				subagent_type: Type.String({
					description: `The type of specialized agent to use. Available types: ${getAvailableTypes().join(", ")}. Custom agents from .cheap/agents/*.md (project) or ${getAgentDir()}/agents/*.md (global) are also available.`,
				}),
				model: Type.Optional(
					Type.String({
						description:
							'Model to use for this agent. Pick from the models listed in Your Team based on task complexity. Format "provider/modelId" (e.g. "cheap-dev/minimax-m2.7") or fuzzy name ("kimi", "minimax", "nemotron"). In orchestration mode you MUST always specify this.',
					}),
				),
			...
			renderResult(result, { expanded: piExpanded, isPartial }, theme, context) {
				// Wire cheap's global ctrl+o cycle: register on first render, then read the
			...
			execute: async (toolCallId, params, signal, onUpdate, ctx) => {
				...
				// LLMs and personas cannot create hidden agents. Internal cheap callers (e.g. permission
	...
	const projectAgentsDir = () => join(process.cwd(), ".cheap", "agents")
	...
	async function showGenerateWizard(ctx: ExtensionCommandContext, targetDir: string) {
		...
		const generatePrompt = `Create a custom cheap sub-agent definition file based on this description: "${description}"
...
models: <optional ordered list of models, e.g. ["cheap-dev/minimax-m2.7"]. Omit to inherit parent model>
...
Write the file using the write tool. Only write the file, nothing else.`
	...
	async function showManualWizard(ctx: ExtensionCommandContext, targetDir: string) {
		...
		if (wizardModelChoices.includes(modelChoice)) {
			modelLine = `\nmodels: ["${CHEAP_DEV_PROVIDER}/${modelChoice}"]`
