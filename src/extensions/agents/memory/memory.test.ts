...
const FAKE_AGENT_DIR = join(homedir(), ".config", "cheap", "harness")
...
// env var is PI_CODING_AGENT_DIR. In production (the cheap binary), cheap's
// package.json sets piConfig.name=cheap, so the var becomes CHEAP_CODING_AGENT_DIR.
describe("resolveMemoryDir", () => {
	...
	it("project scope resolves under <cwd>/.cheap/agent-memory/", () => {
		const cwd = "/some/project"
		const dir = resolveMemoryDir("my-agent", "project", cwd)
		expect(dir).toBe(join(cwd, ".cheap", "agent-memory", "my-agent"))
	})

	it("local scope resolves under <cwd>/.cheap/agent-memory-local/", () => {
		const cwd = "/some/project"
		const dir = resolveMemoryDir("my-agent", "local", cwd)
		expect(dir).toBe(join(cwd, ".cheap", "agent-memory-local", "my-agent"))
	})
