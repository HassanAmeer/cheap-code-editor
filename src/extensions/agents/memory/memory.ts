 ...
 *
 * Memory scopes:
 *   - "user"    → ~/.config/cheap/harness/agent-memory/{agent-name}/
 *   - "project" → .cheap/agent-memory/{agent-name}/
 *   - "local"   → .cheap/agent-memory-local/{agent-name}/
 */
...
export function resolveMemoryDir(agentName: string, scope: MemoryScope, cwd: string): string {
	...
	switch (scope) {
		...
		case "project":
			return join(cwd, ".cheap", "agent-memory", agentName)
		case "local":
			return join(cwd, ".cheap", "agent-memory-local", agentName)
	}
}
