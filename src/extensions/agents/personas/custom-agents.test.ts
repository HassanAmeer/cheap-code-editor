...
const FAKE_AGENT_DIR = join(tmpdir(), `cheap-global-custom-agents-${Date.now()}`)
...
describe("AgentConfig.tokenBudget parsing", () => {
	...
	beforeEach(() => {
		...
		projectDir = join(tmpdir(), `cheap-project-token-budget-${Date.now()}`)
		projectAgentsDir = join(projectDir, ".cheap", "agents")
	})
...
describe("custom agents — user override hierarchy preserves new fields", () => {
	...
	beforeEach(() => {
		tmpRoot = join(tmpdir(), `cheap-hierarchy-${Date.now()}`)
