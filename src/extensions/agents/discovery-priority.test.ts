...
const FAKE_AGENT_DIR = join(tmpdir(), `cheap-global-${Date.now()}`)
...
describe("discovery-priority: project agents override global", () => {
	...
	it("project .cheap/agents/ overrides global agent with same name", () => {
		const cwd = join(tmpdir(), `cheap-project-${Date.now()}`)
		...
		const projectAgentsDir = join(cwd, ".cheap", "agents")
	...
	it("global agent is returned when no project override exists", () => {
		const cwd = join(tmpdir(), `cheap-global-only-${Date.now()}`)
	...
	it("project agent dirs use .cheap not .pi", () => {
		const cwd = join(tmpdir(), `cheap-path-check-${Date.now()}`)
		const projectAgentsDir = join(cwd, ".cheap", "agents")
		writeAgentMd(projectAgentsDir, "path-check-agent", "cheap path agent")
