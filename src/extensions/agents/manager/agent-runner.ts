...
/** Cached cheap config — loaded once per process to avoid repeated disk reads per agent spawn. */
const cheapConfig = loadConfig()
...
async function runAgentInner(
	...
	if (debugSession) {
		try {
			const debugDir = join(effectiveCwd, ".cheap", "debug", debugSession)
	...
	// apply persona-specific path scopes (e.g. plan persona → .cheap/plans/).
	const prevPersona = process.env.CHEAP_AGENT_PERSONA
	if (agentConfig?.name) {
		process.env.CHEAP_AGENT_PERSONA = agentConfig.name
	}
	...
	try {
		...
		if (agentConfig?.name) {
			...
			if (prevPersona === undefined) {
				...
				delete process.env.CHEAP_AGENT_PERSONA
			} else {
				process.env.CHEAP_AGENT_PERSONA = prevPersona
			}
		}
		if (personaPhase) {
