...
vi.mock("./config.js", async (importOriginal) => {
	...
	return {
		...mod,
		getSuperpowersVendorDir: () => join(process.env.HOME ?? "", ".config", "cheap", "vendor", "superpowers"),
	}
})
...
beforeEach(() => {
	...
	mkdirSync(join(mockHome, ".config", "cheap", "vendor"), { recursive: true })
})
...
describe("ensureSuperpowersInstalled", () => {
	...
	it("returns false when already installed at correct version", async () => {
		const vendorDir = join(mockHome, ".config", "cheap", "vendor", "superpowers")
	...
	it("cleans up tarball on download stream failure", async () => {
		...
		const tarballPath = join(mockHome, ".config", "cheap", "vendor", "superpowers.download.tar.gz")
