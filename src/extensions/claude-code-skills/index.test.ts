...
describe("Claude Code skills extension", () => {
	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "cheap-claude-code-skill-tool-"))
	...
	it("does not load native skills through the Claude-compatible Skill tool", async () => {
		writeSkill(
			join(dir, "project", ".cheap", "skills", "best-practices", "SKILL.md"),
			"Cheap project skill instructions.",
		)
		writeSkill(
		...
		expect(textResult(result)).not.toContain("Cheap project skill instructions.")
	...
	it("contributes sanitized Claude Code skills through resources_discover", async () => {
		...
		expect(result).toMatchObject({
			skillPaths: [expect.stringContaining("cheap-claude-code-skills-")],
		})
