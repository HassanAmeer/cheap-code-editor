...
describe("Claude Code hook adapter integration", () => {
	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "cheap-claude-code-hook-adapter-"))
...
if (process.env.CHEAP_HOOK_EVENT !== "PreToolUse") process.exit(2)
if (process.env.CHEAP_TOOL_NAME !== "Bash") process.exit(2)
...
`,
		...
		expect(pi.sendMessage).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				customType: "cheap-claude-code-hook-context",
				display: false,
			}),
