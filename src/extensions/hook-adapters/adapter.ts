...
async function runPreToolUse(
	...
	if (result.updatedInput) Object.assign(event.input, cheapToolInput(result.updatedInput, event.input))
...
function hookEnv(payload: Record<string, unknown>, extra?: Record<string, string>): NodeJS.ProcessEnv {
	...
	return {
		...process.env,
		...extra,
		CHEAP_HOOK_EVENT: eventName,
		CHEAP_TOOL_NAME: toolName,
	}
}
