 ...
 * `CHEAP_CODING_AGENT_DIR` and re-imports the cache module so the memoized
 ...
 * `CHEAP_CODING_AGENT_DIR`.
 */
async function buildHarness(): Promise<Harness> {
	tempHome = mkdtempSync(join(tmpdir(), "mcp-cache-test-"))
	process.env.CHEAP_CODING_AGENT_DIR = tempHome
...
beforeEach(() => {
	originalEnv = process.env.CHEAP_CODING_AGENT_DIR
})

afterEach(() => {
	if (originalEnv === undefined) delete process.env.CHEAP_CODING_AGENT_DIR
	else process.env.CHEAP_CODING_AGENT_DIR = originalEnv
	if (tempHome) {
