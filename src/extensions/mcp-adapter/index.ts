		...
		promise
			.then(async (nextState) => {
				...
				try {
					...
					const { strategy, bm25K1, bm25B, fieldWeights } = cheapConfig.mcpSearch
	...
	if (shouldRegisterProxyTool) {
		pi.registerTool({
			...
			async execute(
				...
				try {
					const cheapConfig = loadConfig()
					maxToolResultChars = cheapConfig.maxToolResultChars
				} catch {
				...
				try {
					const cheapConfig = loadConfig()
					mcpSearchLimit = cheapConfig.mcpSearchLimit
				} catch {
