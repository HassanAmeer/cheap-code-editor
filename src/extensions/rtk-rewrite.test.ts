...
describe("rewriteWithRtk", () => {
	...
	let previousCheapDir: string | undefined
	let previousHome: string | undefined

	beforeEach(() => {
		_resetRtkState()
		previousCheapDir = process.env.CHEAP_CODING_AGENT_DIR
		previousHome = process.env.HOME
		tmpRtkRoot = mkdtempSync(join(tmpdir(), "cheap-rtk-rewrite-test-"))
		process.env.CHEAP_CODING_AGENT_DIR = tmpRtkRoot
	...
	afterEach(() => {
		...
		if (previousCheapDir === undefined) delete process.env.CHEAP_CODING_AGENT_DIR
		else process.env.CHEAP_CODING_AGENT_DIR = previousCheapDir
		// biome-ignore lint/performance/noDelete: env-var cleanup needs a real delete.
...
describe("rtkSpawnHook", () => {
	...
	beforeEach(() => {
		...
		previousCheapDir = process.env.CHEAP_CODING_AGENT_DIR
		previousHome = process.env.HOME
		tmpRtkRoot = mkdtempSync(join(tmpdir(), "cheap-rtk-spawn-test-"))
		process.env.CHEAP_CODING_AGENT_DIR = tmpRtkRoot
		process.env.HOME = tmpRtkRoot
		mockExecFileSync.mockReset()
		mockExecFile.mockReset()
