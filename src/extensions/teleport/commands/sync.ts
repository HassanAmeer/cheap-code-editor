...
export async function runSync(rawArgs: string, ctx: TeleportContext): Promise<void> {
	...
	if (!ctx.apiKey) {
		refuse(ctx, "No API key configured. Run `cheap login`.")
	}
	...
	try {
		...
		try {
			creds = await authenticateWorkspace(workspace.id, ctx.apiKey, workspace.name ?? (basename(ctx.cwd) || "cheap"), {
...
const SYNC_WIDGET_KEY = "cheap-sync-progress"
const SPIN_FRAMES = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
const SPIN_MS = 100
