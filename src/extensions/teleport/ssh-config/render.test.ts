...
vi.mock("../provisioning/proxy-command.js", () => ({
	buildProxyCommand: (target: string) => `cheap --ssh-proxy ${target}`,
}))
...
describe("renderSshConfig", () => {
	...
	it("notes an empty file when no workspaces are provisioned", () => {
		...
		expect(out).not.toContain("Host cheap-")
	})

	it("renders one block per provisioned workspace with the session-id ProxyCommand", () => {
		const out = renderSshConfig(
			[ws({ id: "uuid-aaaa-1111", name: "My Project", host: "ws-aaa.remote.cheap.dev" })],
			FIXED_NOW,
		)
		expect(out).toContain("Host cheap-my-project")
		expect(out).toContain("    HostName ws-aaa.remote.cheap.dev")
		expect(out).toContain("    User sandbox")
		expect(out).toContain("    ProxyCommand cheap --ssh-proxy uuid-aaaa-1111")
	...
	it("skips workspaces without a host", () => {
		...
		expect(out).toContain("Host cheap-ready")
		expect(out).not.toContain("Host cheap-pending")
		expect(out).not.toContain("Host cheap-blank-host")
	})
	...
	it("disambiguates colliding slugs with an id suffix", () => {
		...
		expect(out).toMatch(/Host cheap-my-project-uuidaaaa\b/)
		expect(out).toMatch(/Host cheap-my-project-uuidbbbb\b/)
	})
	...
	it("falls back to the id prefix when the name is empty", () => {
		...
		expect(out).toContain("Host cheap-abcdef01")
	})
	...
	it("is deterministic — sorts by alias", () => {
		...
		expect(a).toBe(b)
		expect(a.indexOf("cheap-alpha")).toBeLessThan(a.indexOf("cheap-zeta"))
	})
})
