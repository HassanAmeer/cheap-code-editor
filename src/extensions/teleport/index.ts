...
function makeHandler(run: CommandFn) {
	return async (args: string, ctx: ExtensionCommandContext): Promise<void> => {
		...
		const tctx: TeleportContext = {
			apiKey,
			endpoint: process.env.CHEAP_REMOTE_ENDPOINT,
			cwd: ctx.cwd,
			signal: ctx.signal,
			ui: ctx.ui,
...
export default function teleportExtension(pi: ExtensionAPI): void {
	if (process.env.CHEAP_EXPERIMENTAL_TELEPORT === undefined || process.env.CHEAP_EXPERIMENTAL_TELEPORT === "") {
		return
	}
	pi.registerCommand("teleport", {
		description: "Open a cheap PTY session in a sandbox workspace",
	...
	pi.registerCommand("sessions", {
		description: "List cheap sessions across all workspaces",
		handler: makeHandler(runSessions),
	})
	pi.registerCommand("workspaces", {
		description: "List and manage cheap workspaces",
		handler: makeHandler(runWorkspaces),
	})
	pi.registerCommand("ssh-config", {
		description: "Refresh ~/.config/cheap/ssh_config from your workspaces",
