...
describe("GENERAL_TIPS", () => {
	it("uses concrete Cheap workflows instead of generic placeholders", () => {
		...
		expect(messages).toContain("Resume the latest session with `cheap --continue`.")
		expect(messages).toContain("Use `cheap --verbose` when output looks off.")
	...
	it("fits every built-in tip in an 80-column row without truncation", () => {
		for (const tip of GENERAL_TIPS) {
			const [line] = renderTipRow({ ...tip, source: "cheap.general" } as TipCandidate, plainTheme, 80)
