...
function makeTip(id: string, message: string, source = "cheap.general"): TipCandidate {
...
describe("sourceToLabel", () => {
	it("maps known sources", () => {
		expect(sourceToLabel("cheap.general")).toBe("General")
		expect(sourceToLabel("cheap.ferment")).toBe("Ferment")
	})

	it("strips cheap prefix and capitalizes unknown sources", () => {
		expect(sourceToLabel("cheap.custom")).toBe("Custom")
	})

	it("capitalizes sources without cheap prefix", () => {
...
describe("createTipsPanel", () => {
	describe("render", () => {
		it("renders numbered tips grouped by source", () => {
			const tips = [
				makeTip("a", "First tip.", "cheap.general"),
				makeTip("b", "Second tip.", "cheap.general"),
				makeTip("c", "Ferment tip.", "cheap.ferment"),
			]
	...
	describe("grouping", () => {
		it("groups tips from different providers under separate headers", () => {
			const tips = [
				makeTip("a", "General tip.", "cheap.general"),
				makeTip("b", "Ferment tip.", "cheap.ferment"),
				makeTip("c", "Custom tip.", "cheap.custom"),
			]
			const panel = createTipsPanel(tips, plainTheme(), makeTui(), vi.fn())
		...
		it("hides empty groups when search filters out all their tips", () => {
			const tips = [
				makeTip("a", "Press shift to do things.", "cheap.general"),
				makeTip("b", "Ferment specific tip.", "cheap.ferment"),
			]
			const panel = createTipsPanel(tips, plainTheme(), makeTui(), vi.fn())
