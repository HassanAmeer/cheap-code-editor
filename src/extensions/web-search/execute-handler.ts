...
export const SEARCH_ENDPOINT = "https://llm.cheap.dev/v1/search"
...
export async function executeWebSearch(params: WebSearchParams, signal?: AbortSignal): Promise<WebSearchResult> {
	...
	if (!apiKey) {
		throw new Error(
			"Web search requires an API key. Run 'cheap' and log in, or visit https://app.cheap.dev to create a key.",
		)
	}
