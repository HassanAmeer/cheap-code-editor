 ...
 * (detected via Agent worker context or the legacy CHEAP_SUBAGENT env var).
 *
 * Main model mode:
...
function writeMultiModelSetting(enabled: boolean): void {
	try {
		...
		const dir = join(homedir(), ".config", "cheap", "harness")
...
export default function (skillPaths: string[]) {
	return (pi: ExtensionAPI) => {
		...
		pi.registerFlag("debug-prompts", {
			...
			default: process.env.CHEAP_DEBUG_PROMPTS === "1",
		})
		...
		pi.on("resources_discover", (event) => {
			const skillPaths = getCheapProjectSkillPaths(event.cwd)
		...
		pi.on("before_agent_start", async (event, ctx) => {
			...
			if (cachedSkills === undefined) {
				...
				const allSkillPaths = Array.from(
					new Set([
						...getCheapProjectSkillPaths(ctx.cwd),
			...
			const env: EnvironmentInfo = {
				...
				cwd: ctx.cwd,
				documentsDir: join(ctx.cwd, ".cheap", "docs"),
			...
			// e.g. orchestrators embedding cheap as a managed agent.
			const appendSystemPrompt = event.systemPromptOptions?.appendSystemPrompt?.trim()
			if (appendSystemPrompt) {
				systemPrompt = `${systemPrompt}\n\n${appendSystemPrompt}`
			}

			const debugSession = process.env.CHEAP_DEBUG_SESSION
			const debugFlag = pi.getFlag("debug-prompts") === true
			if (debugFlag || debugSession) {
				const sessionId = debugSession ?? randomUUID().slice(0, 8)
				process.env.CHEAP_DEBUG_PROMPTS = "1"
				process.env.CHEAP_DEBUG_SESSION = sessionId

				const debugDir = join(ctx.cwd, ".cheap", "debug", sessionId)
				...
				process.env.CHEAP_DEBUG_PROMPTS = undefined
				process.env.CHEAP_DEBUG_SESSION = undefined
			}
