 ...
 *
 * Both prefer cheap-dev models, walking tiers light → standard → heavy.
 * Only when no cheap-dev model is available do they fall back to the
...
export function resolveClassifierModels(modelRegistry: ModelRegistry): ClassifierModels | undefined {
	const primary = resolveCheapModel(modelRegistry) ?? cheapestAvailable(modelRegistry)
	if (!primary) return undefined

	const fallback = resolveCheapModel(modelRegistry, primary.id) ?? cheapestAvailable(modelRegistry, primary.id)

	return { primary, fallback }
}

/**
 * Find the best available cheap-dev model, excluding `excludeId`.
 * Walks tiers light → standard → heavy and returns the first non-empty tier's
 * cheapest model.
 */
function resolveCheapModel(modelRegistry: ModelRegistry, excludeId?: string): Model<Api> | undefined {
	for (const tier of TIERS_ASCENDING) {
		for (const [id, caps] of MODEL_CAPABILITIES) {
			...
			const resolved = modelRegistry.find(CHEAP_DEV_PROVIDER, id)
