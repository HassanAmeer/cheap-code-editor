...
function cheap(id: string, cost = { input: 10, output: 30, cacheRead: 0, cacheWrite: 0 }): Model<Api> {
	return fakeModel(CHEAP_DEV_PROVIDER, id, cost)
}

/** Registry that resolves cheap-dev models by ID from a map, and returns a fixed available list. */
function fakeRegistry(cheapModels: Map<string, Model<Api>> = new Map(), available: Model<Api>[] = []): ModelRegistry {
	return {
		find: (provider: string, id: string) => (provider === CHEAP_DEV_PROVIDER ? cheapModels.get(id) : undefined),
...
describe("resolveClassifierModels", () => {
	...
	describe("primary", () => {
		it("picks the cheap light model regardless of current provider", () => {
			const light = cheap(LIGHT_ID)
		...
		it("falls back to standard when no light model resolves", () => {
			const standard = cheap(STANDARD_ID)
			const current = fakeModel("anthropic", "claude-opus")
		...
		it("falls back to cheapest available when no cheap models resolve", () => {
	...
	describe("fallback", () => {
		...
		it("falls back to cheapest available when no cheap model available for fallback", () => {
			const light = cheap(LIGHT_ID)
