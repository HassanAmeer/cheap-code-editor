...
describe("subagent inherits parent session permission mode", () => {
	...
	it("reads mode from CHEAP_PERMISSIONS_<parentSessionId> when CHEAP_PARENT_SESSION_ID is set", async () => {
	...
	it("parent session key takes precedence over base CHEAP_PERMISSIONS", async () => {
	...
	it("falls back to config default when neither CHEAP_PERMISSIONS nor parent session key is set", async () => {
		Reflect.deleteProperty(process.env, PERMISSIONS_ENV_KEY)
		Reflect.deleteProperty(process.env, PARENT_SESSION_ID_ENV_KEY)
