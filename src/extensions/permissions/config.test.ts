...
beforeEach(() => {
	tmpCwd = mkdtempSync(join(tmpdir(), "cheap-perm-test-"))
})
...
describe("loadConfig merging", () => {
	it("reads project config and tags rules by source", () => {
		mkdirSync(join(tmpCwd, ".cheap"), { recursive: true })
		writeFileSync(
			join(tmpCwd, ".cheap", "permissions.json"),
			JSON.stringify({
				defaultMode: "plan",
				allow: ["bash(git:*)"],
				deny: ["write(.env)"],
	...
	it("merges local on top of project", () => {
		...
		writeFileSync(
			join(tmpCwd, ".cheap", "permissions.local.json"),
	...
	it("cli override replaces merged config entirely", () => {
		mkdirSync(join(tmpCwd, ".cheap"), { recursive: true })
		writeFileSync(join(tmpCwd, ".cheap", "permissions.json"), JSON.stringify({ allow: ["bash(git:*)"] }))
	...
	it("accepts yolo as defaultMode and round-trips it", () => {
		mkdirSync(join(tmpCwd, ".cheap"), { recursive: true })
		writeFileSync(join(tmpCwd, ".cheap", "permissions.json"), JSON.stringify({ defaultMode: "yolo" }))
