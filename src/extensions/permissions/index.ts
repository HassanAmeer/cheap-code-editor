...
/**
 * Check whether a file path is within .cheap/plans/ relative to cwd.
 * Accepts both relative paths (starting with .cheap/plans/) and
 * absolute paths under <cwd>/.cheap/plans/.
 * Path traversal is prevented by resolving to an absolute path first.
 */
export function isWithinCheapPlans(filePath: string, cwd: string): boolean {
	const normalizedCwd = cwd.endsWith("/") ? cwd : `${cwd}/`
	const plansDir = `${normalizedCwd}.cheap/plans/`
...
// `set_phase` is a cheap built-in. `agent`/`get_subagent_result`/`steer_subagent`
...
export default function permissionsExtension(pi: ExtensionAPI): void {
	...
	// CHEAP_PERMISSIONS_<sessionId> and advertises the session ID via
	// CHEAP_PARENT_SESSION_ID. If this variable is absent, we fall back to the
	// base key.
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		// Plan persona path-scope enforcement: when CHEAP_AGENT_PERSONA=plan (case-insensitive),
		// write and edit are only allowed for .cheap/plans/* paths.
		if (process.env.CHEAP_AGENT_PERSONA?.toLowerCase() === "plan") {
			if (toolName === "write" || toolName === "edit") {
				const filePath =
					typeof input.file_path === "string" ? input.file_path : typeof input.path === "string" ? input.path : ""
				if (filePath && !isWithinCheapPlans(filePath, ctx.cwd)) {
					return {
						block: true,
						reason: `Plan persona: ${toolName} is restricted to .cheap/plans/ files. The path "${filePath}" is outside that scope.`,
					}
				}
				// path is within .cheap/plans/ — allow without further checks
				return undefined
			}
		}
