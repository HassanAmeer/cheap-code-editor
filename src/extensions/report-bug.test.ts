...
describe("reportBugExtension", () => {
	...
	it("registers the 'bug' command", () => {
		...
		expect(commands.get("bug")?.description).toBe("Report a bug in cheap — opens GitHub issue form")
	})
	...
	it("builds URL with required params when called without args (UI mode)", async () => {
		...
		expect(url).toMatch(/^https:\/\/github\.com\/getcheap\/cheap\/issues\/new\?/)
	...
	it("falls back to console.log in headless mode without opening browser", async () => {
		...
		expect(console.log).toHaveBeenCalledWith(expect.stringContaining("https://github.com/getcheap/cheap/issues/new"))
