...
describe("autoInitFromEnv", () => {
	const originalVal = process.env.CHEAP_AUTO_GIT_INIT

	afterEach(() => {
		if (originalVal === undefined) {
			process.env.CHEAP_AUTO_GIT_INIT = undefined
		} else {
			process.env.CHEAP_AUTO_GIT_INIT = originalVal
		}
	})

	it("returns false when CHEAP_AUTO_GIT_INIT is not set", () => {
		process.env.CHEAP_AUTO_GIT_INIT = undefined
		expect(autoInitFromEnv()).toBe(false)
	})

	it("returns true when CHEAP_AUTO_GIT_INIT is '1'", () => {
		process.env.CHEAP_AUTO_GIT_INIT = "1"
		expect(autoInitFromEnv()).toBe(true)
	})

	it("returns false when CHEAP_AUTO_GIT_INIT is not '1'", () => {
		process.env.CHEAP_AUTO_GIT_INIT = "yes"
		expect(autoInitFromEnv()).toBe(false)
	})
})
