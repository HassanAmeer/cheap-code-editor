...
const ORIGINAL_PI_MANAGER = Symbol.for("cheap.originalPiPackageManager")

type PackageScope = "user" | "project"
type SourceScope = PackageScope | "temporary"
type PackageOrigin = "cheap" | "pi"
...
export function getOriginalPiAgentDir(): string {
	const explicitAgentDir = process.env.CHEAP_ORIGINAL_PI_CODING_AGENT_DIR
	if (explicitAgentDir) return resolveTilde(explicitAgentDir)

	const piAgentDir = process.env.PI_CODING_AGENT_DIR
	if (piAgentDir && !isCheapAgentDir(piAgentDir)) return resolveTilde(piAgentDir)

	return resolveTilde(join(homedir(), ORIGINAL_PI_CONFIG_DIR_NAME, "agent"))
}

export function isOriginalPiPackageLookupEnabled(settingsPath = cheapSettingsPath()): boolean {
	try {
...
export function getOriginalPiConfiguredPackages(cwd = process.cwd()): ConfiguredPackageEntry[] {
	if (!isOriginalPiPackageLookupEnabled() || isOriginalPiAgentDirSameAsCheap()) return []
...
export async function resolveOriginalPiPackageResources(
	...
	if (!isOriginalPiPackageLookupEnabled() || isOriginalPiAgentDirSameAsCheap()) return emptyResolvedPaths()
...
function isOriginalPiAgentDirSameAsCheap(): boolean {
	try {
		return resolve(getOriginalPiAgentDir()) === resolve(getCheapAgentDir())
	} catch {
		return false
	}
}

function getCheapAgentDir(): string {
	return process.env.CHEAP_CODING_AGENT_DIR
		? resolve(process.env.CHEAP_CODING_AGENT_DIR)
		: join(homedir(), ".config", "cheap", "harness")
}

function isCheapAgentDir(path: string): boolean {
	try {
		return resolveTilde(path) === getCheapAgentDir()
	} catch {
		return false
	}
}

function cheapSettingsPath(): string {
	return join(getCheapAgentDir(), "settings.json")
}
