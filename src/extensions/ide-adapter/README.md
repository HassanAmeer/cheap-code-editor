...
Generic adapter that discovers IDE plugins via lockfiles and exposes their tools to Cheap over the Model Context Protocol (MCP).
...
1. **Discovery**: Scans `$XDG_CONFIG_HOME/cheap/ide/` for `*.lock` files written by IDE plugins.
