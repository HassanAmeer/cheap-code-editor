...
// Any theme without these values (including cheap-minimal) lets the terminal
// own its bg/fg.
//
...
// theme bg lingers under another theme or cheap-minimal.
//
...
function getThemeColors(themeName: string): { fgHex: string; bgHex: string } | null {
	try {
		const dir = process.env.CHEAP_CODING_AGENT_DIR
		if (!dir) return null
...
export default function terminalColorsExtension(pi: ExtensionAPI) {
	...
	const reactToThemeChange = (newName: string | undefined) => {
		...
		lastCtx?.ui.setStatus("cheap-theme-rerender", undefined)
	}
	...
	pi.on("session_start", (_event, ctx) => {
		...
		ctx.ui.setWidget("cheap-osc-sensor", () => ({
			invalidate(): void {
