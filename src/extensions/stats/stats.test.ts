...
describe("getProviderDisplayName", () => {
	...
	it("maps pi-otel to Cheap", () => {
		expect(getProviderDisplayName("pi-otel")).toBe("Cheap")
	})
...
describe("getSourceName", () => {
	...
	it("maps pi-otel to Cheap", () => {
		expect(getSourceName("pi-otel")).toBe("Cheap")
	})
...
describe("sortFn", () => {
	const fixtures = [
		...
		{ modelName: "gpt-4", source: "Cheap", cost: 8, inputTokens: 800, outputTokens: 400 },
	]
	...
	describe("sort by cost", () => {
		...
		it("uses modelName then source as tiebreaker", () => {
			const tied = [
				...
				{ modelName: "gpt-4", source: "Cheap", cost: 10, inputTokens: 200, outputTokens: 200 },
			...
			expect(sorted[1].source).toBe("Cheap")
	...
	describe("sort by tokens", () => {
		...
		it("uses modelName then source as tiebreaker", () => {
			const tied = [
				...
				{ modelName: "gpt-4", source: "Cheap", cost: 2, inputTokens: 1000, outputTokens: 1000 },
			]
			const sorted = [...tied].sort((a, b) => sortFn(a, b, "tokens"))
			expect(sorted[0].source).toBe("Cheap")
	...
	describe("sort by model", () => {
		...
		it("uses source as tiebreaker for same model", () => {
			...
			expect(gpt4Entries[0].source).toBe("Cheap")
...
describe("formatAnalyticsVisual", () => {
	...
	it("coerces string int64 token values from protobuf JSON encoding", () => {
		...
		expect(aggregated.perModel.get("gpt-4\tCheap")?.inputTokens).toBe(1000)
		expect(aggregated.perModel.get("gpt-4\tCheap")?.outputTokens).toBe(500)
		expect(aggregated.perModel.get("claude-3\tCheap")?.inputTokens).toBe(2000)
		expect(aggregated.perModel.get("claude-3\tCheap")?.outputTokens).toBe(1000)
	})
})
...
describe("formatAnalyticsSummary", () => {
	...
	it("coerces string int64 token values from protobuf JSON encoding", () => {
		...
		expect(aggregated.totals.totalInput).toBe(3000)
		expect(aggregated.totals.totalOutput).toBe(1500)
		expect(aggregated.perModel.get("gpt-4\tCheap")?.inputTokens).toBe(3000)
		expect(aggregated.perModel.get("gpt-4\tCheap")?.outputTokens).toBe(1500)
	})
})
