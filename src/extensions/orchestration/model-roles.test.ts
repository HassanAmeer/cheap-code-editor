...
describe("parseModelRoles", () => {
	...
	it("ignores unknown keys", () => {
		...
		expect(roles.orchestrator).toBe("cheap-dev/kimi-k2.6")
		expect(warnings).toHaveLength(0)
	})
...
describe("splitModelRef", () => {
	it("splits a valid provider/model-id string", () => {
		expect(splitModelRef("cheap-dev/kimi-k2.6")).toEqual({
			provider: "cheap-dev",
			modelId: "kimi-k2.6",
		})
	})
...
describe("modelIdFromRef", () => {
	it("extracts model ID from provider/model-id", () => {
		expect(modelIdFromRef("cheap-dev/kimi-k2.6")).toBe("kimi-k2.6")
	})
...
describe("DEFAULT_MODEL_ROLES", () => {
	it("orchestrator is minimax-m3", () => {
		expect(DEFAULT_MODEL_ROLES.orchestrator).toBe("cheap-dev/minimax-m3")
	})

	it("builder pool contains the build role but not nemotron", () => {
		const builders = normalizeRoleModels(DEFAULT_MODEL_ROLES.builder)
		expect(builders).toContain("cheap-dev/minimax-m3")
		expect(builders).not.toContain("cheap-dev/nemotron-3-ultra-fp4")
	})
...
describe("saveModelRoles", () => {
	const testDir = join(tmpdir(), `cheap-model-roles-test-${process.pid}`)
...
describe("parseModelRoles rejects objects", () => {
	...
	it("warns on array containing objects", () => {
		const { roles, warnings } = parseModelRoles({
			planner: ["cheap-dev/kimi-k2.6", { model: "anthropic/claude-opus-4-6" }],
		})
