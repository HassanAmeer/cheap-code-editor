 ...
 * - Per-project storage: ferments live in .cheap/ferments/ inside the repo, or global fallback
 */
...
export function getGlobalFermentsDir(): string {
	return resolve(homedir(), ".config", "cheap", "ferments")
}
...
export function resolveFermentsDir(cwd?: string): string {
	...
	const envDir = process.env.CHEAP_FERMENTS_DIR
	if (envDir) return envDir
	const project = detectProjectRoot(cwd)
	if (project) return resolve(project, ".cheap", "ferments")
