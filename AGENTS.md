# Agent Guidelines for Cheap-Dev

You are editing the cheap-code CLI harness. This repo extends the pi-mono SDK (`@earendil-works/pi-coding-agent`) — core agent loop lives upstream; this repo adds extensions in `src/extensions/`.
...
- **Structure TUI tests through the shared fixture.** Use `runCheapSession`, deterministic fake OpenAI responses, isolated temp `HOME`/workdir, and trace steps for meaningful checkpoints. Avoid brittle ANSI/snapshot assertions unless the rendering itself is the behavior under test.
...
## Documents Directory
- `.cheap/docs/` → Transient AI working files — git-ignored, do NOT commit
