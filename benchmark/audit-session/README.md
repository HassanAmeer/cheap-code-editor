...
Audits a completed cheap harness session, analyzing phase discipline, code quality, model alignment, and cost efficiency. Produces a graded report written to `.cheap/audits/`.
...
```sh
# Interactive session picker, default runner (cheap)
...
# Custom model
./benchmark/audit-session/audit-session.sh -m cheap-dev/kimi-k2.6
...
| `-r`, `--runner CMD` | Harness to use: `cheap` (default) or `claude` |
| `-m`, `--model MODEL` | Model override (default: `cheap-dev/claude-opus-4-6` for cheap, `claude-opus-4-6` for claude) |
| `-h`, `--help` | Show help |
...
|--------|------|-------------|---------------|
| `cheap` | interactive, yolo | all tools allowed | `cheap-dev/claude-opus-4-6` |
...
The audit report is written to `.cheap/audits/{sessionId}-{task}-{runner}-{model}-AUDIT.md` in the project directory. It includes:
...
```
.cheap/audits/session-20260511-124841-complex-AUDIT.md
```
...
4. Launches the chosen runner (cheap or claude-code) in interactive mode with the prompt
...
| File | Description |
|------|-------------|
| `.cheap/audits/{sessionId}-{task}-{runner}-{model}-AUDIT.md` | Human-readable graded report |
| `.cheap/audits/{sessionId}-{task}-{runner}-{model}-AUDIT.json` | Machine-readable sidecar for automated comparison |
...
jq -s '.[0].aggregates.totalCost as $a | .[1].aggregates.totalCost as $b |
  ...
  .cheap/audits/session-AUDIT.json .cheap/audits/session-BUDIT.json
...
jq -s 'map(.aggregates.totalCost) | add' .cheap/audits/*-AUDIT.json
```

## Prerequisites

- `cheap` and/or `claude` CLI available on PATH
- `jq` recommended (used for session parsing and JSON sidecar validation; falls back to grep)
- Session JSONL files (from `~/.config/cheap/harness/sessions/` or `benchmark/manual/`)
