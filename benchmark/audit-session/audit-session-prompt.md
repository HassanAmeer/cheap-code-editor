...
7. Read project context files (if they exist in the session's cwd):
   - README.md
   - CLAUDE.md
   - .cheap/ directory contents
```

For production code evidence:
```
8. Find new/modified source files produced during this session:
    git log --oneline --since="<session_start_timestamp>" -- ':!.gsd/' ':!.cheap/'
    git diff --stat <first_session_commit>..HEAD -- ':!.gsd/' ':!.cheap/'
...
Write the audit report to: `.cheap/audits/{auditFilename}`

Create the `.cheap/audits/` directory if it does not exist.
...
Write a new file at `.cheap/audits/{auditFilename}` containing:
...
After writing the JSON sidecar, say: "Audit complete for session {sessionId}. Report at .cheap/audits/{auditFilename}"
