...
SESSIONS_BASE="${HOME}/.config/cheap/harness/sessions"
...
Audit a cheap harness session for phase quality and cost efficiency.
...
Options:
  ...
  -r, --runner CMD    Harness to use: cheap or claude (default: cheap)
  -m, --model MODEL   Model to use (default: cheap-dev/claude-opus-4-6)
  -s, --session PATH  Write session JSONL to PATH (cheap only)
  -h, --help          Show this help

Examples:
  $(basename "$0")                              # pick session, run with cheap
...
case "$RUNNER" in
    cheap)  EFFECTIVE_MODEL="${MODEL:-cheap-dev/claude-opus-4-6}" ;;
    claude)  EFFECTIVE_MODEL="${MODEL:-claude-opus-4-6}" ;;
    *)       echo "Unknown runner: $RUNNER (use 'cheap' or 'claude')" >&2; exit 1 ;;
esac
...
echo "Audit file: .cheap/audits/$AUDIT_FILENAME"
echo ""
...
run_audit_agent() {
    ...
    case "$runner" in
        cheap)
            local -a cheap_args=(--model "$model" --yolo)
            if [[ -n "$SESSION_OUT" ]]; then
                cheap_args+=(--session "$SESSION_OUT")
            fi
            cheap_args+=("@${prompt_file}")
            cheap "${cheap_args[@]}"
            ;;
        claude)
...
echo ""
audit_file=".cheap/audits/$AUDIT_FILENAME"
