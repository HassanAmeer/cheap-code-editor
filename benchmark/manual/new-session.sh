...
BINARY=$(python3 -c "import json,os; cfg=json.load(open('$BENCHMARK_JSON')); print(os.path.expanduser(cfg.get('binary','~/_dev/cheap-dev/dist/bin/cheap')))")
...
DIR=$(mktemp -d /private/tmp/cheap-{slug}-XXXXXX)
