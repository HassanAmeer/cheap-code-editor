...
Teleport is available when the `CHEAP_EXPERIMENTAL_TELEPORT` environment variable is set:

```bash
CHEAP_EXPERIMENTAL_TELEPORT=1 cheap
```
...
- **An API key** — run `cheap login` if you haven't already.
...
|---|---|
| `CHEAP_REMOTE_ENDPOINT` | Override the remote API endpoint (advanced/internal use). |
| `CHEAP_API_KEY` | Provide an API key without running `cheap login`. |
...
When you exit the SSH session (`exit` or Ctrl-D), you return to cheap.
...
When `--source` is a directory, default excludes (`node_modules/`, `dist/`, `.next/`, `target/`, `__pycache__/`, `.venv/`, `.env*`, `*.log`, `.DS_Store`, `.cheap/`, …) are applied, plus your repo's gitignored entries unless `--include-ignored` is set. When `--source` is a single file, only explicit `--exclude` patterns are applied.
...
| `Ctrl+D` | Exit the overlay entirely and return to local cheap |
...
When a session terminates normally (e.g. you type `exit` in the shell), its tab closes. If that was the last tab, the overlay exits and you return to local cheap. A tab closed by a fatal error stays in the tab bar so you can see the banner and decide whether to retry or exit.

---

## Git credentials prompt

When you run `/teleport` and cheap detects a git remote (e.g. `github.com`), it shows an interactive prompt asking for a personal access token. This token is forwarded to the sandbox so the remote agent can push and pull from private repositories.
...
If you previously saved a token for the detected host, cheap uses it automatically. To skip the prompt entirely, use `/teleport --no-git-token`.

On subsequent `/terminal` calls or when reusing a workspace, cheap automatically propagates saved credentials if they haven't been synced yet.
...
If you reference a session that no longer exists, cheap will suggest using `/sessions` to browse available sessions. Sessions that have finished and been cleaned up server-side cannot be reattached.
