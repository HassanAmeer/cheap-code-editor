...
Cheap's `agents` extension brings Claude-Code-style subagents to the harness. The
...
Cheap has a separate phase system that gates the **parent session's** autonomy
(read-only vs full edit, etc.). Phases and agents are independent:

- **Phases** (managed by cheap's `behaviours` extension and `set_phase` tool)
...
|---|---|---|
| 1 (highest) | `<cwd>/.cheap/agents/<name>.md` | Project — per-repo agents |
| 2 | `~/.config/cheap/harness/agents/<name>.md` | User — available everywhere |
...
| `Explore` | Explore | `cheap-dev/nemotron-3-super-fp4` | read-only | Fast codebase exploration |
| `Plan` | Plan | `cheap-dev/minimax-m2.7` | read-only | Architecture and implementation planning |
| `Researcher` | Researcher | `cheap-dev/kimi-k2.6` | read-only + web | Web and docs research with cited sources |
...
- **`models`** — Set of models this persona may use, e.g. `["cheap-dev/nemotron-3-super-fp4", "cheap-dev/minimax-m2.7", "cheap-dev/kimi-k2.6"]`. **List order has no semantics** — it is not a tier ranking. The set is shown to the calling LLM as part of the `Agent` tool description; the LLM picks per spawn using its knowledge of each model's tier/strengths (from the orchestration model registry). If the caller omits `model`, the runtime falls back to the first entry as a stable default — this is **not** a complexity-aware pick, so for non-trivial tasks always pass an explicit `model`. Omit `models` entirely to inherit the parent session's model. For default agents, cheap resolves `models` from the strength registry at startup; see `default-agents.ts` for the strength-tag mapping.
...
- **`skills`** — `true` (default) inherits all parent skills. A comma-list
  ...
  `.cheap/skills` directory are available automatically.
- **`memory`** — Enables a persistent directory keyed by agent name and scope:
  - `user` → `~/.config/cheap/harness/agent-memory/<name>/`
  - `project` → `<cwd>/.cheap/agent-memory/<name>/`
  - `local` → `<cwd>/.cheap/agent-memory-local/<name>/`
...
`.cheap/agents/adaptive-reader.md`:
...
tools: read, grep, find, ls
models:
  - cheap-dev/nemotron-3-super-fp4   # default — fast, light
  - cheap-dev/minimax-m2.7           # medium
  - cheap-dev/kimi-k2.6              # heavy
thinking: medium
---
...
If a session is interrupted while an `Agent` call is in flight, cheap will
...
runtime exposes the path via the `CHEAP_AGENT_MEMORY_DIR` environment variable
inside the agent's tool calls. Conventional layout:

```
.cheap/agent-memory/<agent-name>/
