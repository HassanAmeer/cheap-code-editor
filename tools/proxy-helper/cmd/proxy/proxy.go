...
import (
	...
	"github.com/castai/cheap/tools/proxy-helper/pkg/cast"
...
func NewSSHProxyCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "SSH ProxyCommand bridge to a Cheap remote sandbox",
		Long: `Acts as an SSH ProxyCommand bridge to a Cheap remote sandbox.

Requires $CHEAP_API_KEY to be set.
Override the API endpoint with $CHEAP_REMOTE_ENDPOINT.

Example ~/.ssh/config entry:

  Host *.remote.cheap.dev
    ProxyCommand cheap-ssh-proxy ssh-proxy %h`,
