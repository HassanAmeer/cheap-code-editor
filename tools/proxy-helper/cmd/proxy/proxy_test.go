...
// fakeBackend stands in for the Cheap API + the sandbox WS endpoint.
...
func TestReadAPIKey(t *testing.T) {
	t.Setenv("CHEAP_API_KEY", "env-key")
	...
	t.Setenv("CHEAP_API_KEY", "")
	if _, err := cast.ReadAPIKey(); err == nil {
		t.Error("expected error when CHEAP_API_KEY is unset")
	}
}
