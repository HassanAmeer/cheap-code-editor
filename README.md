...
A coding agent CLI powered by [cheap](https://cheap.dev/). Built on the [pi-mono](https://github.com/badlogic/pi-mono) coding agent SDK, cheap gives you an AI-powered development assistant in your terminal that connects to cheap's LLM infrastructure.
...
```bash
brew install getcheap/tap/cheap
```

**Install script:**

```bash
curl -fsSL https://github.com/getcheap/cheap/releases/latest/download/install.sh | bash
```

Then configure your API key and launch:

```bash
cheap setup   # one-time interactive setup
cheap         # launch the coding agent
```

Run `cheap --help` to see all available subcommands and flags.

## Models

### Model selection

The supported model list is fetched at startup from the cheap metadata service. Use `/model` or `ctrl+p` in the interactive CLI to switch between available models.

Cheap operates in one of two modes:
...
Use `/multi-model` in the interactive CLI to toggle models on/off per role, or edit `~/.config/cheap/harness/settings.json` directly:

```json
{
  "modelRoles": {
    "orchestrator": "cheap-dev/kimi-k2.6",
    "builder": ["cheap-dev/minimax-m2.7", "anthropic/claude-sonnet-4-5"],
...
Built-in models (cheap-dev) have tier and description baked in. External models need metadata — see below.
...
Cheap tags every LLM request with a `phase:{name}` label for usage analytics and cost attribution. The orchestrator sets the phase as work progresses and it is displayed in the footer.
...
Cheap supports tagging LLM requests for usage tracking and cost attribution. Tags are included with every request and displayed in the footer, grouped by key with color coding.
...
Set via the `CHEAP_TAGS` environment variable (comma-separated). Static tags are read-only within the session and shown with a `[static]` marker.

```bash
export CHEAP_TAGS="team:backend,project:api"
```
...
User-defined tags (added via `/tags add`) are persisted to `~/.config/cheap/tags.json` and survive across sessions. Static tags from `CHEAP_TAGS` must be set each session.

## Ferment -- cross-session project management

Ferment is Cheap's progressive-refinement project mode for multi-session work. Instead of starting from scratch each chat, Ferment persists a structured plan (goal, phases, steps) as a JSON state file.

### Quick start

```bash
cheap --ferment "Build Tetris"
```
...
cheap ships with built-in Language Server Protocol (LSP) support, giving the agent type-aware code intelligence. The extension loads by default — no configuration required.
...
Launch with `cheap --teleport` to enable session-multiplex commands. The local TUI stays the home base; remote workers are spawned, detached, and re-attached without restarting cheap.
...
1. `CHEAP_API_KEY` environment variable (takes precedence)
2. `~/.config/cheap/config.json` field `api_key`

Run `cheap setup` for an interactive first-time configuration.

### Agent config

Cheap stores its configuration (settings, sessions, models) under:

```
~/.config/cheap/harness/
```

### Context files

You can provide custom instructions that are injected into the system prompt on every session. Cheap discovers two kinds of context files:

**Global** — applied to every session, regardless of project:

```
~/.config/cheap/harness/AGENTS.md
```
...
**Project-level** — applied when working in a specific directory tree. Cheap walks from the working directory up to the filesystem root and collects one context file per directory:
...
Cheap supports native Pi packages. `cheap install npm:<package>` installs a package only for Cheap, while `pi install npm:<package>` keeps the package owned by the original Pi harness. Cheap can also load original Pi packages through the **Pi package lookup** resource, so both CLIs can use Pi packages without sharing Cheap's install scope.

If the same package is installed in both places, the Cheap install wins. Disable original Pi package lookup with:

```bash
cheap resources disable extensions.pi-package-lookup
```

Update packages with:

```bash
cheap update                  # update installed packages, then Cheap itself
cheap update --extensions     # update installed packages only
cheap update context-mode     # update one package by source or display name
cheap update self             # update Cheap itself only
```

### HTTP proxy

Cheap respects `HTTP_PROXY` / `HTTPS_PROXY` environment variables for network requests.

### Token optimization (RTK)

Cheap installs [RTK](https://github.com/rtk-ai/rtk) during setup and keeps the `rtk` command available on startup. When enabled, cheap rewrites bash tool calls through `rtk rewrite` before execution. This compresses command output (git, cargo, npm, docker, etc.) by 60-90%, reducing LLM context usage.

Before every bash tool execution, cheap calls `rtk rewrite "<command>"`. If RTK returns a rewritten command (e.g. `git status` becomes `rtk git status`), the rewritten version is executed instead.
...
cheap resources enable hooks.rtk-rewrite
```

### Hooks

Users can add custom Bash hooks to rewrite or block shell commands before they run. Global hooks live in `~/.config/cheap/harness/hooks/bash/`; project hooks live in `.cheap/hooks/bash/` and default to disabled until enabled from `/resources` or `cheap resources enable ...`.

See `docs/hooks.md` for the hook protocol and examples.

### Migrating from another coding agent

On first run, cheap looks for an existing **Claude Code**, **OpenCode**, or **Cursor** installation and offers to migrate its MCP servers. If anything is migratable you will see a one-shot prompt:
...
|
*  Migrate MCP servers to Cheap?
|  * Migrate now
|  * Skip this time
|  * Never ask again
```

Discovered MCP servers are merged into `~/.config/cheap/harness/mcp.json`. Existing Cheap entries always win on name collisions, so re-running the migration is safe.
...
3. **Against existing Cheap config**: your entries in `~/.config/cheap/harness/mcp.json` always win.

#### "Never ask again"

Stored in `~/.config/cheap/config.json` (`migrationState: "skip-forever"`). Delete that field to re-trigger the prompt. Adding support for another agent is a small change -- drop a new definition into `src/agent-discovery/agents/` and append it to the registry.
...
```bash
git clone git@github.com:getcheap/cheap.git
cd cheap
corepack enable
pnpm install
```
...
The `benchmark/` directory contains tools for smoke-testing cheap sessions and auditing their quality.
...
- **Terminal-bench-2** (`benchmark/terminal-bench-2/`) -- run the [terminal-bench](https://www.harborframework.com/) suite (89 tasks) against cheap inside Docker containers. See `benchmark/terminal-bench-2/README.md`.
...
Release assets follow the naming convention `cheap_{os}_{arch}.tar.gz` with a `checksums.txt` (SHA256) for verification.
